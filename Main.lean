...
+ undefined-column)"
  ...
  -- EITHER direction.  It was ELEVEN LOW — and eleven low is exactly the error
...
# x86lean coverage\n\n\
Roster: " ++ toString rosterSize ++ " mnemonics in " ++ toString vectors.length ++ " differentially tested forms, covering **464 of the 525 rows** in `p1/roster.tsv` — which are **322 of the 382 distinct machine forms** those rows describe, because 143 rows are alias SPELLINGS of another row (`jz` for `je`, `sal` for `shl`, `xchg ax,r` for `xchg r,ax`). Of the 464, **346 are spelled by a vector** and 118 are the same encoding under a different spelling. ⭐ ALL FOUR NUMBERS ARE DERIVED, by `scripts/claimed_forms.py`, and gated in CI; until P1 batch 19 the first was a hand-maintained literal and it was ELEVEN LOW.\n\n\
...
MATCHES, so a mis-parse dies rather than counting. The literal was ELEVEN LOW, \
