...
def wrongCvtWith (keep : Bool) (fl il : BitVec 64 → BitVec 64) (i : Instr) (s : Cpu) : Cpu :=
  let nr := s.rip + BitVec.ofNat 64 i.len
  let low (fi : Bool) (d : BitVec 128) (b : BitVec 64) : BitVec 128 :=
    (if keep then (d >>> 64) <<< 64 else 0) ||| (if fi then il b else fl b).setWidth 128
  match i.op with
  | .vcvtss2sd dst src =>
...
/-- ⛔ THE UPPER HALF OF THE DESTINATION IS ZEROED, as `movsd` does from memory.
The legacy SSE rule keeps it; this is invisible wherever it was zero. -/
def wrongCvtZeroesUpper : Instr → Cpu → Cpu :=
  wrongCvtWith false SoftFloat.f32to64 SoftFloat.i32to64

/-- ⛔ THE int32 IS READ UNSIGNED: right on every non-negative source. -/
def wrongCvtUnsigned : Instr → Cpu → Cpu :=
  wrongCvtWith true SoftFloat.f32to64 (fun x =>
    let v := x.setWidth 32
    if v == 0 then 0 else SoftFloat.toBinary64 false (v.setWidth 64) 1023)

/-- ⛔ ONLY 24 SIGNIFICANT BITS SURVIVE, as if the integer had gone through a
binary32 first (truncated here rather than rounded; either is wrong exactly
where the magnitude needs more than 24 bits). -/
def wrongCvtSingleSignificand : Instr → Cpu → Cpu :=
  wrongCvtWith true SoftFloat.f32to64 (fun x =>
    ...
    if v == 0 then 0
      ...
      SoftFloat.toBinary64 neg m' 1023)
...
def wrongCvtIntMinSaturates : Instr → Cpu → Cpu :=
  wrongCvtWith true SoftFloat.f32to64 (fun x =>
    if x.setWidth 32 == 0x80000000#32 then SoftFloat.toBinary64 true 0x7FFFFFFF 1023
    else SoftFloat.i32to64 x)

/-- ⛔ THE WHOLE 64-BIT REGISTER IS CONVERTED, as if REX.W were set (the low
bits of a wide magnitude truncated).  Right wherever the upper half of the
source is the sign extension of the lower; `%ecx` differs from that in 40
states.  A memory source reads four bytes either way, so only the register
form is planted.

⚠️ It encodes the value itself rather than calling `SoftFloat.toBinary64`,
whose top-bit search is 32 bits wide and would mis-encode this magnitude — a
wrong model should be wrong in the way its docstring says, not in a second way. -/
  ...
  | .vcvtsi2sd dst src =>
      ...
      let r : BitVec 64 :=
        if v == 0 then 0
          ...
          let neg := v.getLsbD 63
          let m := if neg then -v else v
          let p := 63 - Value.clzN m 64
          let mant := (if p ≤ 52 then m <<< (52 - p) else m >>> (p - 52)) &&& ((1 <<< 52) - 1)
          (if neg then (1 : BitVec 64) <<< 63 else 0) |||
            (BitVec.ofNat 64 (1023 + p) <<< 52) ||| mant
      (s.setXmm dst ((((s.getXmm dst) >>> 64) <<< 64) ||| r.setWidth 128)).setRip nr
  | _ => step i s
