...
`ordered` bit, `mm` a min/max's, `cvt` a `cvtss2sd`'s, and `trunc` a truncation's.  With
`Cpu.withSimd`, `SoftFloat.preFlags` (quiet NaNs signalling for `cmp` only when `ordered`, always
for `mm`, never for `cvt`) and `SoftFloat.truncFlags`, it is `step` restated.  Every other form,
the integer packed kinds included, is `step`. -/
def wrongSimdWith (raise : Cpu → BitVec 32 → (Cpu → Cpu) → Cpu)
    ...
    (trunc : SoftFloat.Fmt → Nat → BitVec 64 → BitVec 32) (i : Instr) (s : Cpu) : Cpu :=
  let nr := s.rip + BitVec.ofNat 64 i.len
...
def wrongSimdReplaces : Instr → Cpu → Cpu :=
  wrongSimdWith (fun s fl k => s.withSimd fl fun t => k { t with mxcsr := (s.mxcsr &&& ~~~0x3F#32) ||| fl })
    simdCmp simdMm simdCvt SoftFloat.truncFlags
...
def wrongSimdCvttDropsPE : Instr → Cpu → Cpu :=
  wrongSimdWith Cpu.withSimd simdCmp simdMm simdCvt
    (fun f w x => SoftFloat.truncFlags f w x &&& ~~~SoftFloat.fPE)
