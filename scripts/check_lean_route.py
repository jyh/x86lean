...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="x86lean-checkroute-") as tmp:
        ...
        open(os.path.join(tmp, DOOR), "w").write('X = ["lake", "build"]\n')
        open(os.path.join(tmp, "scripts", "ok.sh"), "w").write("python3 scripts/lean_route.py build X\n")
        open(os.path.join(tmp, "scripts", "bad.py"), "w").write('import subprocess\nsubprocess.run(["lake", "build"])\n')
        open(os.path.join(tmp, "untracked.sh"), "w").write("lake build\n")
