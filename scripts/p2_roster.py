...
def mnemonic_verdicts(forms, refuted):
    """({mnemonic: verdict}, {conflicting mnemonics}) over the probe table.

    ⛔⛔ D257 — THE MNEMONIC IS THE `asm` COLUMN'S FIRST TOKEN, NOT THE LABEL.  This
    loop read `label.split("_")[0]`, which agrees with the assembled mnemonic on 260
    of the table's 262 probe rows and disagrees on exactly two: the rows labelled
    `cvtsi2sd`/`cvtsi2ss` assemble `cvtsi2sdl %ecx`/`cvtsi2ssl %ecx`.  While the
    census keyed register-source conversions by the bare spelling the mismatch
    happened to join; once the census keys them by WIDTH, a label-keyed verdict
    lands on a key no demand carries and 1,941 instructions fall out of "the oracle
    EXECUTES".  `oracle_availability.measured_availability` has read the asm token
    since D100 for exactly this reason — ONE RULE, NOW AT BOTH READERS.
    """
    ...
    verdict = {}
    conflict = set()
    for label, asm, _b, _e0, e1 in forms:
...
""")
    ...
    for ck, occ in dem.items():
        ...
        if route == "vector":
            vec[mn] += occ
    verdict, conflict = mnemonic_verdicts(OA.P2_FORMS, OA.REFUTED_BY_MEASUREMENT)
...
def selftest():
    ...
    # ⭐ D257: the by-mnemonic verdict is keyed by the ASSEMBLED mnemonic
    class _R:
        e1 = "refuses"
    plant = [("cvtsi2sd", "cvtsi2sdl %ecx, %xmm0", "", "refuses", "executes"),
             ("CONTROL_x", "nop", "", "executes", "executes"),
             ("movq_xmm", "movq %xmm0, %rax", "", "refuses", "executes"),
             ("movq_mmx", "movq %mm0, %rax", "", "refuses", "refuses"),
             ("orps_r", "orps %xmm1, %xmm0", "", "refuses", "executes")]
    pv, pc = mnemonic_verdicts(plant, {"orps_r": _R()})
    live, _lc = mnemonic_verdicts(OA.P2_FORMS, OA.REFUTED_BY_MEASUREMENT)
    for name, ok in [
        ("a probe LABELLED `cvtsi2sd` that assembles `cvtsi2sdl` is keyed `cvtsi2sdl`",
         pv.get("cvtsi2sdl") == "executes" and "cvtsi2sd" not in pv),
        ("a CONTROL row carries no verdict", "nop" not in pv),
        ("two probes of one mnemonic that disagree are a CONFLICT, read as refuses",
         pc == {"movq"} and pv.get("movq") == "refuses"),
        ("a refutation overrides the sealed verdict", pv.get("orps") == "refuses"),
        ("the live table keys both width-suffixed conversions and no bare one",
         all(live.get(k) == "executes" for k in
             ("cvtsi2sdl", "cvtsi2ssl", "cvtsi2sdq", "cvtsi2ssq"))
         and "cvtsi2sd" not in live and "cvtsi2ss" not in live)]:
        say(("  ✔ " if ok else "  ⛔ ") + name)
        if not ok:
            bad.append("verdict-key:" + name)
    if bad:
