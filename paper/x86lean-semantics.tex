% x86lean: the semantics paper (paper 1). TACAS 2027 case-study track (ruled 2026-09-13), LNCS format.
%
...
first. It covers 197 mnemonics, where K's coverage target lists 774, and its proof support is a safety
...
against the AST in both directions, so a row cannot name a mnemonic the AST lacks and a mnemonic cannot
lack a row, and the published coverage document is regenerated from the table and compared in CI.
...
because the manual leaves them undefined, and \emph{absent} when the model refuses the mnemonic entirely; forms it refuses within a row are
listed in that row's shapes. Of 197 mnemonics, 171 are exact, 26 are frame, and 0 are absent. The tier names are this project's coinage.
\src{X86/Coverage.lean inductive Tier; Tests/Coverage.lean table\_mnemonics\_subset\_roster and roster\_covered\_by\_table compare mnemonic
lists; refused forms inside non-absent rows: docs/COVERAGE.md bswap, shld, bt (D250); docs/COVERAGE.md:13; docs/CLAIMS.tsv[coverage\_mnemonics, tier\_exact, tier\_frame,
tier\_absent]}

\paragraph{Trust.}
Decoding is trusted, not proved. The AST is produced from Intel XED's structured output, and every one
of the 197 rows records its decoding as trusted to XED; a Lean decoder for the covered subset is later
work.
...
The model would have been self-consistent under either reading. The differential run of
Section~\ref{sec:validation} agrees with this one: at such counts x86lean's CF is compared rather than
excused, and it matched the reference model on every case; a control that flips CF in one such record is
contradicted.
\src{docs/DECISIONS.md D20; docs/DIFFERENTIAL-P1-BATCH7.md section 3, the planted control on sar\_b9/6 (a flipped record, not a model copy);
under the other reading CF would be in the derived undefined set and a disagreement would be explained, so the run could not have
refuted it by a disagreement (D250)}
...
the oracle cursor's position after particular instructions, one of which shows it equal under two
opposite oracles, and we have not proved it for every instruction.
\src{Tests/Nonvacuity.lean cursor\_add\_spends\_nothing, cursor\_and\_spends\_one, cursor\_shift\_nonzero\_spends\_three,
cursor\_shift\_zero\_spends\_nothing, cursor\_independent\_of\_bits (the one over two oracles, one SHL at one state); docs/DECISIONS.md D213, D250}
...
the cases that draw, 27{,}717 of 85{,}008 in the check's own walk of 84 pre-states per vector.
\src{docs/CURSOR-LEAK-CHECK-2026-09-13.md, the cursor-plant line and the 84-vs-88 pre-state note (D250); docs/CLAIMS.tsv[cursor\_cases, cursor\_leaks, cursor\_plant\_fired, cursor\_plant\_cases, cursor\_walk\_prestates]; X86/Serialize.lean
...
Below a 64-bit operand SHLD and SHRD mask their count to five bits, so at a 16-bit operand the count can
exceed the width and the destination becomes undefined. This is not a corner: over the 82 pre-states of the harness at the
time, a count taken from CL exceeded 16 in 35.
\src{docs/DECISIONS.md D52, measured on the reference model before the constructor existed; D52's ``five bits (six at .q)''; docs/CLAIMS.tsv[d52\_prestates, d52\_count\_over16]}
...
The current record covers 1{,}152 vectors at 88 pre-states, 101{,}376 cases, of which 80{,}679 are
matched. The remaining cases carry 29{,}479 field disagreements explained by the undefined set and 292
declared divergences. 0 disagreements are unexplained, and in 0 cases did an undefined bit leak.
\src{docs/DIFFERENTIAL-P2-BATCH36.md, the RUN block; docs/CLAIMS.tsv[diff\_vectors, diff\_prestates, diff\_cases,
...
An earlier record, of 1{,}012 vectors, was re-run with the reference model pinned at ACL2 commit c8897a34, and
the re-run reproduced every counter: 89{,}056 cases, 68{,}524 matched, 29{,}435 explained, 171 declared
divergences, 0 unexplained and 0 leaks. The current record was itself taken against that pinned commit. That
re-run's leak count was taken before the leak check compared the oracle cursor, and the current record's
was taken after; the cursor's own count is the separate record of Section~\ref{sec:undef:draw}.
\src{docs/REFERENCE-PIN-RUN-2026-09-12.md; scripts/oracle\_revision.txt; docs/CLAIMS.tsv[pin\_reference\_model\_short, pin\_vectors, pin\_cases,
...
A paper whose argument is that claims about a semantics should be checked must also report where the
check has nothing to check against. Five findings fall there.
...
did not: it executed both an unaligned load and an unaligned store, while an aligned MOVDQA, an
unaligned MOVDQU and a control instruction behaved as expected.
\src{docs/DECISIONS.md D91 1, at a reference-model revision the record does not name (scripts/oracle\_revision.txt: earlier records
do not record their tree); D250}
x86lean faults, as the manual says, and no unaligned MOVDQA vector is shipped, since every one would be
scored as a one-sided refusal although x86lean is right. The rule is therefore carried by theorem
...
MOVDQA is the example of a class: the same fault on the legacy SSE memory forms is carried by theorem
for most of them, because the reference model implements it only for PAND, POR and PXOR, where three
vectors test it. This is the weakest claim in the model: it rests on our reading of the manual and on a
proof that the model implements that reading, and nothing else.
\src{docs/DECISIONS.md D113 (three of nineteen validatable; the other sixteen and vload, vstore, vshufm, vshiftm theorem-only);
X86/Theorems.lean vbinm\_unaligned\_faults; D250}
...
source. They are most of the declared count: 163 of the current record's 292. The eight shifts below are
another eight, and the floating-point batches added the remaining 121. A wrong model planted to merge rather than
...
eight shifts were declared; docs/CLAIMS.tsv[movdq\_divergences, diff\_diverged, fp\_divergences\_added, p2\_shift\_divergences, liblisa\_machines]; 292 = 171 + the four floating-point records' increases (DIFFERENTIAL-P2-BATCH27, 29, 31 and 35: +50, +11, +12, +48; every other record since BATCH22 adds 0), D300; docs/LIBLISA-HARDWARE-CHECK-2026-09-12.md, the vmovd and vmovq verdicts; D221; docs/DECISIONS.md D248}
...
docs/CLAIMS.tsv[liblisa\_shift\_verdicts\_pass, liblisa\_shift\_forms, liblisa\_machines]; docs/DECISIONS.md D221}
...
\paragraph{A reference model whose floating point is not the processor's.}
The rounding forms (scalar and packed arithmetic, conversions and square roots) share one rounding
rule, and there the reference model departs from the manual in ways the manual alone does not settle.
An invalid operation returns the floating-point indefinite without its sign bit. A conversion from single
to double precision returns an overflow result whenever the sticky overflow flag is already set, because
the reference reads its own accumulated flags as an input. Converting the integer 0 under round-down
returns $-0$. So a processor decides each case: every such row is executed on two vendors' processors, an
AMD EPYC 7763 and an Intel Core i7-8700B, one instruction per row, before any Lean is written, and the row
is then pinned in the kernel as a \texttt{decide} theorem over x86lean's semantics. The sign-less
indefinite and the signed zero are declared divergences, each at exactly its shape. The overflow defect
never reaches the differential, because no pre-state presets that flag, and is carried by the pinned rows.
One instruction is left out. On SQRTPS, every source in the populated pre-states puts a negative value in
a lane above the lowest, and a divergence entry can excuse only the lowest lane, so SQRTPS has a
definition and processor rows but is not claimed.
\src{docs/DECISIONS.md D266 4 (the preset-OE and zero-sign defects, at cvt-spec.lisp), D265 and D284 (rtl::indef,
scalar and packed), D267 10 (the Intel reading beside the AMD runner's), D291 (vsqrt on roundPack); X86/SoftFloat.lean roundPack, and roundsUp, its decision rule, which cvtToInt calls directly, D295 2 (SQRTPS);
hwprobe/; Tests/Anchors.lean, the cvtss2sd\_*\_sticky rows marked ``x86isa differs''; Main.lean knownDivergences}
...
362{,}093 instructions are forms the model does not cover.
...
demand actually occurs, it executes the forms carrying 102{,}931 of them and refuses the forms carrying
169{,}877; on 241, in three mnemonics, it neither advances nor refuses; and 88{,}882 have not yet been
...
p2\_bucket\_executes, p2\_bucket\_refuses, p2\_bucket\_stalls, p2\_bucket\_stalls\_pairs, p2\_bucket\_unasked, p2\_ceiling\_partition\_residue];
162 + 102931 + 169877 + 241 + 88882 = 362093}
Among those whose form has been asked, the reference model therefore refuses more than it executes, both
counted by instructions and counted by forms (88 forms refused, 67 executed). The unasked share is larger
than the difference in instructions, so the comparison does not yet extend to the whole gap.
\src{docs/P2-ROSTER.md, the BY (mnemonic, BUCKET) table's pairs column, one form asked per pair; docs/CLAIMS.tsv[p2\_bucket\_pairs\_executes,
p2\_bucket\_pairs\_refuses]; this sentence said ``Among the forms it has been asked'' until 2026-09-15, true only by instruction, D250; and until the 2026-09-24 re-pin (D300) it said more forms executed (106) than refused (88), which the covered batches since have reversed}
...
\paragraph{Coverage.}
The row and form figures are computed from two sources that are not derived from each other. One is each
...
The published figures are 197 mnemonics in 1{,}152 differentially tested vectors, read from the table
the model generates, and 500 of the 525 roster rows, which are 351 of the 374 distinct machine forms those
rows describe; CI fails if the prose that states them disagrees with the derivation. The roster is K's
scalar forms: the model's vector forms have no row there, are listed by name as exempt, and are counted
against a separate demand roster instead.
\src{scripts/claimed\_forms.py, the mnemonic count read from the generated COVERAGE.md, and CLAIMS\_NO\_ROW (204 SIMD entries of 234);
111 of the 197 table rows take an xmm operand (README.md:59, at the pinned sha); docs/COVERAGE.md:5 and README.md:56-57, checked against the derivation by scripts/claimed\_forms.py --check in CI;
...
an invariant indexed by label. These theorems are stated from an entry state whose pointer registers
are universally quantified and whose other registers are fixed (defaults, and in one routine a count of
four), over every amount of fuel and every initial memory; the two copying routines move a constant four
bytes. Across four routines with 2, 3, 5 and 7 labels, the proofs are 34, 50, 57
...
\src{Tests/Program.lean entry, cpyEntry, frameEntry, guardEntry (rdx := 4); fill and memcpy load rcx with 4; fill\_safe and memcpy\_safe over
(pointers, m0, n) with Region d0 4 (D250);
scripts/label\_fit.py (a label is an entry of the invariant's table, cross-checked against the program's
...
\src{docs/TACAS-PRICING.md 2.1 (the ruling) and G4; docs/TACAS-G1-POSITIONING.md 1c.2; docs/CLAIMS.tsv[proof\_sample\_routines, x86isa\_program\_families]}
...
\src{docs/CLAIMS.tsv[x86isa\_theorem\_forms, x86isa\_program\_families]; docs/TACAS-G1-POSITIONING.md 1c.2: dataCopy/dataCopy.lisp copyData-is-correct; 559 excludes the
603 forms of the utilities library}
The K semantics~\cite{dasgupta-k-x86,k-x86-repo} counts ``3155 instruction variants, corresponding to
...
\src{docs/TACAS-G1-POSITIONING.md 1c.5 and 1c.7}
Against these, x86lean covers 197 mnemonics where K's target lists 774; x86isa's figure is stated in
...
197 is derived by scripts/claimed\_forms.py into COVERAGE.md, and this copy is checked against the row (D231); docs/DECISIONS.md D220}
...
We have used libLISA's models for the two rules on which x86lean and the reference model part, the
packed-shift count and MOVD/MOVQ's upper bits (Section~\ref{sec:validation:limits}); a comparison of x86lean against them beyond those rules is the most
...
We have read these two papers, not their artifacts, and do not position x86lean against them: Roessle et al.\
count instruction variants rather than mnemonics, and Verbeek et al.\ is about lifting, not an instruction set.
...
Most of x86lean's techniques have public precedents, and we cite them where they originate. A step over one machine state is
the shape of all three models above: a function in x86isa and Sail, rewrite rules in K. The closest precedent in Lean~4 is
...
Section~\ref{sec:model} adds is that the statement is made per mnemonic, as data that is checked against the AST.
...
been asked, the reference model refuses more than it executes, and for the refused ones a model built from
the manual currently has no second source. A rule the reference model does not implement, such as the alignment fault of most legacy SSE memory
forms, is carried by theorem alone. The proof-cost measurements are a sample of six routines. And the
...
itself are, as far as its gates reach, outputs: derived from the artifact, checked against a second source where one exists, and
...
% The category is ruled: case-study track, not double-blind (docs/TACAS-PRICING.md 3a, council 2026-09-13), so
% nothing here is anonymized. The submission commit is not yet fixed, and an archived DOI waits on the repository's
% archive toggle.
...
by \texttt{scripts/setup\_oracle.sh}, which does not pin the host Lisp (the image at that commit was built
with SBCL 2.6.8). The repository's scripts and records call the reference model ``the oracle''; in
this paper that word names x86lean's source of undefined bits.
\src{scripts/setup\_oracle.sh (brew install sbcl, unversioned); vendor/acl2/saved\_acl2 exec line reads sbcl/2.6.8 (D250);
LICENSE; lean-toolchain; scripts/oracle\_revision.txt; docs/CLAIMS.tsv[pin\_reference\_model\_short, proof\_sample\_routines]}
