...
declared divergences; there is no unexplained disagreement and no case in which an undefined bit
leaked.
...
\placeholder{The paper's own numbers are gated against the manifest row each \texttt{src} marker names
only once that check exists (paper/README.md, owed). Until then this paragraph may not claim the paper
is gated.}
...
the claim is not re-driven). Each sentence waits on its read.}
