...
% 2026-09-12, D222-D223). The revision pass is owed (docs/TACAS-PRICING.md 2.2, G7). This comment read
% "Every other section is a placeholder" until then.
...
project's own ungated claims failed a re-check: in each of four samples, at least one time in three.
...
\src{isa-afp.org/entries/X86\_Semantics.html and X86\_InstructionSemantics, locale unknowns, read
...
machine''~\cite{liblisa}. We have read its models only for the packed-shift rule of
Section~\ref{sec:validation:limits}, where they serve as a second source.
...
where the two disagree by the manual and by reading the other model's rules, and for one rule by a model
synthesized on processors (Section~\ref{sec:validation:limits}). That arbiter is fallible
...
\src{liblisa.nl/publications/liblisa-oopsla24/05-results, section 5.2.2 comparison table, row
...
A fourth sample came from writing this paper's related work: of three attributions to prior work that
the project had recorded from the literature and never re-read, two did not survive reading the paper
cited, one crediting a frame rule to a paper whose improvement removes it and one describing as hardware
validation what the paper reports as validation against test suites and simulators.
\src{docs/DECISIONS.md D222 and D223; the third attribution, per-instruction theorems composed without
re-evaluating the model, survived}
In the four samples with a denominator, 3 of 9, 3 of 8, 3 of 7 and 2 of 3, claims that no check read
failed a re-check at least one time in three, and the failures were present when the claims were
written rather than acquired later.
...
labels; the per-label constant was highest on the routine with the most labels, so that figure is more likely low than high.
\src{docs/TACAS-PRICING.md 1, the P2 proof interface block; docs/P2-PROOF-INTERFACE.md, the four-routine fit and its note that the constant rises with label count; counted by
...
\src{Armstrong et al., POPL 2019, section 7 (Validation), read at cl.cam.ac.uk/\textasciitilde pes20/sail/sail-popl2019.pdf
...
That a model should state its own fidelity is not our idea either: the Isabelle/HOL model and the lifting
work above describe theirs as over-approximative~\cite{afp-x86,verbeek-lifting}. What the fidelity tier of
Section~\ref{sec:model} adds is that the statement is made per form, as data that is checked against the AST.
\src{isa-afp.org/entries/X86\_Semantics.html, ``purposefully incomplete, but overapproximative''; the
abstract for DOI 10.1145/3519939.3523702 (api.semanticscholar.org, read 2026-09-12), ``provably overapproximative''; X86/Coverage.lean inductive Tier and
the two-direction table test cited in Section~\ref{sec:model}; docs/DECISIONS.md D223}
...
by reading a second, and for one rule by a model synthesized on processors; K has not been run, and no
hardware co-simulation has been. The reference model
...
general: in this project, claims about formal artifacts that nothing checked failed a re-check at least
one time in three in every sample we took, and the failures were present when the claims were written.
