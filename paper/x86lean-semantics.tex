...
\placeholder{The cost of a divergence entry (it excuses one field for that vector at every pre-state)
and the three things that stand against it, from D108 4.}

\paragraph{The ceiling of the method.}\mbox{}
\placeholder{The reference model cannot execute a large share of the remaining vector instructions at
all, so that share cannot be differentially validated at any price. Quote the live figures from
docs/P2-ROSTER.md at a pinned sha, never D115's dated table (D115 3 says so).}
