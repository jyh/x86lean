...
where the check has nothing to check against, are derived and gated rather than written. We describe a
design for undefined bits that keeps one definition for execution and proof, derives the undefined set
from the semantics instead of declaring it, and states a case the model declines as a theorem; a
...
project's own ungated claims failed a re-check: about one time in three.
...
first. It covers 158 mnemonics, where K's coverage target lists 774, and its proof support is seven
memory-safety theorems over four routines, where x86isa proves full functional correctness of an
unbounded loop. Other formal x86-64 models exist beyond the three we compare against, among them one
with small-step semantics for 1{,}625 instructions built from machine-learned
semantics~\cite{roessle-bigstep}; Section~\ref{sec:related} says what we have and have not read.
\src{docs/TACAS-G1-POSITIONING.md, the scale and proof-support rows and 1d; docs/DECISIONS.md D220}
...
\begin{itemize}
\item an undefined-bit design in which one Lean definition serves execution and proof, the undefined set
  is derived from the semantics rather than declared, and a case the model cannot represent is declined
  by a theorem (Section~\ref{sec:undef});
\item a differential harness whose classes of disagreement, units and wrong-model controls are explicit,
  a statement that its evidence comes from two origins and not three, and the cases where the reference
  model is not the specification (Section~\ref{sec:validation});
\item coverage and validation figures derived from two sources and gated in CI against the prose that
  states them, including those of this paper's own figures that cite the manifest, with a measurement of how often ungated claims
  in this project failed a re-check (Section~\ref{sec:gated});
...
The three public x86-64 semantics whose sources we have read, x86isa~\cite{goel-x86isa,x86isa-repo},
the K semantics~\cite{dasgupta-k-x86,k-x86-repo} and the Sail model translated from
x86isa~\cite{sail-x86-repo}, together with x86lean, answer the question ``what is an undefined bit?'' in four different ways (Table~\ref{tab:undef}). Each
row was read at the project's own source; a fifth design is noted below the table.
...
A fifth public model, an Isabelle/HOL semantics of roughly 120 x86-64 instructions in the Archive of
Formal Proofs, is by its own description ``purposefully incomplete, but overapproximative'': where the
effect of flags is undefined, it maps them ``to universally quantified uninterpreted terms from a
locale''~\cite{afp-x86}. Undefinedness as a parameter is therefore not new with us; we have read that
entry's abstract, not its theories, and do not compare the two designs in detail.
\src{isa-afp.org/entries/X86\_Semantics.html, read 2026-09-12; docs/TACAS-G1-POSITIONING.md 1d; D220}
...
68{,}524 matched, 29{,}435 explained, 171 declared divergences, 0 unexplained and 0 leaks.
...
The three public x86-64 models we compare against could easily be counted as three witnesses. They are
...
an invariant indexed by label. Across four routines with 2, 4, 5 and 7 labels, the proofs are 34, 50, 57
and 76 lines, consistent with about 19 lines plus 7.5 to 8.1 per label, or about 173 lines at twenty
labels.
\src{docs/TACAS-PRICING.md 1, the P2 proof interface block; docs/P2-PROOF-INTERFACE.md; counted by
scripts/proof\_lines.py; docs/CLAIMS.tsv[proof\_lines\_prologue, proof\_lines\_guarded, proof\_lines\_fill,
proof\_lines\_memcpy]. The label counts 2, 4, 5, 7 and the per-label fit are not in the manifest.}
Three successive rounds of interface work on the first of these routines brought its per-label cost
from about 25 lines to 12, 9.6 and 7.6; the third was pre-registered with a refutation threshold, which
...
\src{docs/P2-PROOF-INTERFACE.md, the runP\_code point and round 4}
...
Four routines and two to seven labels, written by one author against one interface, are a sample.
...
\src{docs/TACAS-PRICING.md 2.1 (the ruling) and G4; docs/TACAS-G1-POSITIONING.md 1c.2}
...
We compare against three x86-64 semantics whose sources we have read: two independent of each other,
...
opcodes, a different unit, and Sail's model publishes no count. Its proof support, seven memory-safety
theorems over four routines, is far weaker than x86isa's or K's; the Sail model carries no theorems.
\src{docs/TACAS-G1-POSITIONING.md, the scale and proof-support rows and 1d; 158 is gated by
scripts/claimed\_forms.py; docs/DECISIONS.md D220}
Other formal x86-64 models exist outside this comparison. A public Isabelle/HOL entry in the Archive of
Formal Proofs gives over-approximative semantics for roughly 120 x86-64 assembly instructions, with
symbolic execution of basic blocks and a parser for disassembler output~\cite{afp-x86}. Roessle et al.\ build a machine model with
small-step semantics for 1{,}625 instructions, leveraging machine-learned semantics, and generate
equivalence proofs between decompiled machine code and big-step semantics~\cite{roessle-bigstep};
Verbeek et al.\ lift stripped binaries to a provably over-approximative representation exportable to
Isabelle/HOL~\cite{verbeek-lifting}. We have read their abstracts, not their artifacts, and do not
position x86lean against them.
\src{abstracts via api.semanticscholar.org for DOIs 10.1145/3293880.3294102 and 10.1145/3519939.3523702,
and isa-afp.org/entries/X86\_Semantics.html, read 2026-09-12; docs/DECISIONS.md D220}
...
semantics lifted from binaries (Roessle et al., CPP 2019; Verbeek et al., PLDI 2022 -- abstracts read
09-12, artifacts and availability NOT, D220); the frame discipline in seL4 and AutoCorres; hardware
...
by reading a second; K has not been run, and no hardware co-simulation has been. The reference model
refuses more of the remaining instructions than it executes, so for those a model built from the manual
currently has no second source. A rule the reference model does not implement, such as MOVDQA's
alignment fault, is carried by theorem alone. The proof-cost measurements are a sample of four routines. And the
...
general: in this project, claims about formal artifacts that nothing checked failed a re-check about
one time in three, and the failures were present when the claims were written.
...
it, and \texttt{python3 scripts/check\_claims.py} re-derives all of them and checks the paper's text
...
\src{LICENSE; lean-toolchain; scripts/oracle\_revision.txt; docs/CLAIMS.tsv[pin\_reference\_model\_short]}
