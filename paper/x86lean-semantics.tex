...
\item coverage and validation figures derived from two sources and gated in CI against the prose that
  states them, this paper's manifest-cited figures among them, with a measurement of how often ungated
  claims in this project failed a re-check (Section~\ref{sec:gated});
...
\lean{step\_dshift\_mem\_\allowbreak undefined\_\allowbreak refuses}: for an immediate count, the step leaves the
state unchanged except for the halt reason. The guard in \lean{step} is on the masked count, which a count in CL
reaches as well.
...
The fourth sample with a denominator came from writing this paper's related work: of three attributions to prior work that
...
whose limits are stated as theorems, a differential harness whose disagreement classes and wrong-model
