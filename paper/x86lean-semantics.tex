...
sources and validated by differential testing against ACL2 x86isa. It is smaller than the existing
public x86-64 semantics and weaker in proof support, and we say so first. What it contributes is a
...
unbounded loop. It is the smallest of the three and the weakest on proofs.
\src{docs/TACAS-G1-POSITIONING.md, the scale and proof-support rows and 3; docs/TACAS-PRICING.md 4a}
...
unbounded copy loop, this is the weakest proof development of the three
(Section~\ref{sec:related}). A benchmark of such properties, and a program logic proved sound over this
...
\placeholder{Name the public repository, the commit the paper's figures are pinned at, the manifest and
the one command that re-derives them, the pinned toolchain, and the reference model's revision -- which
is currently neither pinned nor recorded (docs/TACAS-G5-ARTIFACT.md 1), so this statement cannot yet
