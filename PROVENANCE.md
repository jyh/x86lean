...
|---|---|---|---|
| Intel 64 and IA-32 Architectures Software Developer's Manual | intel.com/…/technical/intel-sdm.html (PDF version 092 as of 2026-08-19; its cover reads *Order Number: 325462-092US, June 2026*, Combined Volumes 1–4, re-read 2026-09-13) | Intel copyright; no open licence for the manual text. ⚠️ *The PDF's notice states terms, verbatim:* "No license (express or implied, by estoppel or otherwise) to any intellectual property rights is granted by this document, with the sole exception that a) you may publish an unmodified copy and b) code, identified as Sample Code in this document is licensed subject to the Zero-Clause BSD open source license (0BSD) … You may create software implementations based on this document and in compliance with the foregoing that are intended to execute on the Intel product(s) referenced in this document. No rights are granted to create modifications or derivatives of this document." Recorded, not interpreted: whether that grant reaches a formal model is not claimed here. *This cell read "no open licence stated for the manuals" until 2026-09-13 (D236).* | READING reference. Cited by volume/section; none of its technical text is copied (only the notice, quoted in the licence cell) |
...
| 2026-09-03 (P2 batch 1) | ACL2 x86isa, `machine/segmentation.lisp`, `machine/instructions/move.lisp`, `tools/execution/init-state.lisp` | read BY HAND, three files | © 2015 Regents of the Univ. of Texas (BSD-3) | ⚠️ **Logged under rule 2 even though the semantic rule came from the SDM**, because the reading is real and the honest entry is the one that says exactly what it did and did not decide. What was taken: (a) the DRIVER PLUMBING — that x86isa's 64-bit arm reads FS's and GS's bases from `(msri *ia32_fs_base-idx* …)` rather than from the hidden segment registers, and that `init-x86-state-64`'s fifth argument is the MSR alist. That is how to ASK the oracle, not what the machine does. (b) CORROBORATION, recorded as such in `docs/DIFFERENTIAL-P2-BATCH1.md`: that `x86-lea` uses `x86-effective-addr` and never calls `ea-to-la` while every memory access does. ⭐ **P2 batch 23 adds one file to this row, `machine/dispatch-macros.lisp`, read for the same KIND of thing:** that x86isa models the `f0` prefix at all and raises #UD through `ud-Lock-used` / `ud-Lock-used-Dest-not-Memory-Op`. That reading decided whether the oracle could ANSWER the batch's #UD question — it is about the instrument, not about x86-64 — and the RULE (which forms accept the prefix) is transcribed from SDM Vol. 2A, "LOCK", into `X86/Syntax.lean`'s `Op.lockable` against that citation. The RULE — FS/GS keep a 64-bit base in long mode, and LEA writes the effective address — is from SDM Vol. 3A §3.4.4 and Vol. 2A "LEA", and is stated in `X86/Basic.lean` and `X86/Semantics.lean` against those citations. No Lisp was copied; the model's arithmetic is `Ea.addr`, written in Lean, and the oracle's agreement with it is EXECUTION evidence (67 424 cases) rather than a reading. |
