...
- **Instructions.** 184 mnemonics in 1112 differentially tested forms, covering
  500 of the 525 rows of the P1 roster — the rows are K's grammar of encodable
  forms, and 149 of them are alias spellings of another row. 98 of those
  ...
  `pshufd`/`pshuflw`/`pshufhw` at both operand shapes — and the packed integer
  ...
  `maxsd`/`minps`/`maxps`, the exact widenings `cvtss2sd`/`cvtsi2sdl`, the truncations `cvttsd2si`/`cvttss2si`, the first form that rounds under MXCSR.RC, the scalar multiply `mulss`/`mulsd`, and the scalar add, subtract and divide `addss`/`addsd`/`subss`/`subsd`/`divss`/`divsd`); the P1 roster excludes
...
**P1 SEALED — 21 batches landed. P2 IN PROGRESS — 32 batches landed.** ⭐ **P1's AVAILABLE WORK IS ZERO**: every
...
**500 of the 525 rows** in [`p1/roster.tsv`](p1/roster.tsv) — which are 351 of
