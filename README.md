...
- **Instructions.** 174 mnemonics in 1069 differentially tested forms, covering
  500 of the 525 rows of the P1 roster — the rows are K's grammar of encodable
  forms, and 149 of them are alias spellings of another row. 88 of those
  ...
  `maxsd`/`minps`/`maxps`, the exact widenings `cvtss2sd`/`cvtsi2sdl`, the truncations `cvttsd2si`/`cvttss2si`, and the first form that rounds under MXCSR.RC, the scalar multiply `mulss`/`mulsd`); the P1 roster excludes
...
**P1 SEALED — 21 batches landed. P2 IN PROGRESS — 28 batches landed.** ⭐ **P1's AVAILABLE WORK IS ZERO**: every
