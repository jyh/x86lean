...
| fidelity tier as a per-form datum | x86isa's `app-view`/`sys-view` + its machine-generated catalogue; K's derived support status (§2); the AFP Isabelle/HOL entry's "purposefully incomplete, but overapproximative" semantics. ⛔ *This cell read "Verbeek/Roessle/Bockenek (CPP 2019, PLDI 2022) over-approximative semantics" until 2026-09-14 (D246), and it was wrong for BOTH papers once read: CPP 2019 contains no "overapprox" at all (0 hits; PLDI 34), and PLDI 2022's over-approximation is of a binary's EXECUTION PATHS, with sound instruction semantics taken as an ASSUMPTION (§1)* | the concept is public; **the names T-exact/T-frame/T-absent are our coinage and the plan says so** |
...
| hardware co-simulation | K's gdb-driven `--xstate`/`--kstate`/`--compare`; **Roessle, Verbeek, Ravindran, CPP 2019 §6** (each hardware test case a PROVED Isabelle/HOL lemma about the step function, where x86isa's co-simulation EXECUTES its model: 6,630 test cases per variant, post-states from live Skylake hardware via a Pin tool; 886 variants tested, two failed and were modelled by hand; read at the authors' PDF, D246); **Armstrong et al., POPL 2019** (⛔ NOT "Sail ARM vs hardware", as this cell read until 2026-09-12 — its §7, read at the PDF (D223): ARM's Architecture Validation Suite run on the model and compared with ARM's ASL specification, 24 of 15,400 failing; RISC-V trace-compared with the Spike simulator; no processor); LNSym's `cosim`; x86isa's own co-simulation | ⛔ **already theirs — we may not present it as new** |
...
```
  ...
  a read of Verbeek/Roessle/Bockenek beyond the over-approximation claim           ✅ DONE 09-14 (D246): both papers
                                                                                   at the authors' PDFs; three §7 sentences changed
```
