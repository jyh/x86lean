...
- ✅ **RED-FIRST ARM — BUILT 2026-09-22 (paris, `ku_delta.py` on `paris/ci-one-run-per-sha`): a PER-RUN
  bootstrap.** In `--selftest-measure` the ceiling is a CONTROL, not a gate value. The unplanted tree is
  measured on this box, `max(got × 3, 50)` is derived exactly as the tool already prints it, and the plant
  must then make A′ refuse. It reads no registry, so nothing in it can go stale. **Driven on the dev box
  with the hostname faked to `runnervmtr4k5`**, the VM that drew red on master: control CLEAN on both arms,
  and the plant read X86.Basic **1,780 ms against a per-run ceiling of 277**, so A′ refused it and ARM A
  passed it. PASS, rc 0. The red direction is already on the forge: master run 35742453223 shows the old
  code refusing, exit 2, on exactly that hostname.
  ⚠️ Declared in the code: under per-run ceilings the CONTROL's ms half is clean by construction; the
  control still gates Δku, and the plant is the discriminator.
...
- **Release:** the red-first half is released when this lands. **Owner:** the helm, for the real-gate half. **Re-measure:** at the next landing after #53.
- ✅ **REAL-GATE HALF, 2026-09-24 (D299):** neither recorded arm; the third one. The ceiling on an unregistered box is
  `max(base ms × 3, 50)`, with the base profiled on the same box in the same job. It is not circular because the base is
...
is named at item 4.
⚖️ **BUILT 2026-09-15 (D251), arm (a): the merge rule now lives in `kernel_drift.py --record`, which RECOMPUTES the ms verdict from
the readings and refuses to write a row unless it licenses the step — rc 0 records; rc 1 and rc 2 refuse; rc 3 records only beside an
...
0c. ⛔⛔ **`es3-anchor-theorems` IS BLOCKED ON AN UNDECIDABLE BAND, NOT ON A BUDGET — AND THE GATE'S
   ...
   line: a ruling on whether a `.lean` step may land on an A′ verdict, and what its ledger row records.** OWNER: paris, to post as a design fork. RE-MEASURE: when that ruling lands.
...
3. **The kernel-delta gate** stays the merge gate; the absolute ceilings ride beside every merge as
   ...
   ⚖️ **AMENDED 2026-09-15 (D251):** it stays the merge gate WHERE IT DECIDES; where it is UNMEASURABLE, ARM A′ decides in its place,
   and `kernel_drift.py --record` is now where that rule is enforced (it was enforced nowhere before).
