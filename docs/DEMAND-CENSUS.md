...
<!-- census-model: mnemonics=158 sha=234c181ea435b8d4 rules=180ced9ef7df7612 -->
...
The model covers **158 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,377,696 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,145 (0.0%).
...
| 23 | `cvtsi2sdl` | 6 | 0.00% |
| 24 | `pcmpestri` | 6 | 0.00% |
| 25 | `cmpxchgl (lock operand)` | 6 | 0.00% |

#### Hand-written vs compiler-emitted — the confusion matrix

Symbol maps were found for **0 of 2** objects in this column. ⛔ **No symbol map: this column is NOT attributed**, and every instruction below sits in the `-` cell rather than being counted as compiler output.

| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 5,379,923 | 100.00% | 100.0% |
...
| SSE-legacy (xmm) | 547 | 0.01% | `cvtsi2sd`, `mulsd`, `addsd`, `divsd`, `cvtsi2ss`, `cvtss2sd` |
...
- **Covered by the model: 863,759 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,792 (1.8%).
...
Symbol maps were found for **0 of 106** objects in this column. ⛔ **No symbol map: this column is NOT attributed**, and every instruction below sits in the `-` cell rather than being counted as compiler output.

| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 879,551 | 100.00% | 98.2% |
...
| SSE-legacy (xmm) | 1,775 | 0.20% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 545,131 (90.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 57,751 (9.6%).
...
Symbol maps were found for **0 of 28** objects in this column. ⛔ **No symbol map: this column is NOT attributed**, and every instruction below sits in the `-` cell rather than being counted as compiler output.

| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 603,554 | 100.00% | 90.3% |

#### What is NOT covered, bucketed by ISA extension

| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 15,315 | 2.54% | `mulsd`, `addsd`, `subsd`, `ucomisd`, `comisd`, `ucomiss` |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 303,830 | 100.00% | 64.2% |
...
- **Covered by the model: 2,956,405 (94.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 143,823 (4.6%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 3,116,986 | 100.00% | 94.8% |

#### What is NOT covered, bucketed by ISA extension

| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 49,970 | 1.60% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,947 (98.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,729 (1.1%).
...
| 17 | `cmpnlepd` | 15 | 0.01% |
| 18 | `addsd` | 13 | 0.01% |
| 19 | `cvttps2dq` | 12 | 0.01% |
| 20 | `subsd` | 11 | 0.01% |
| 21 | `cvtdq2ps` | 10 | 0.01% |
| 22 | `mulps` | 10 | 0.01% |
| 23 | `divps` | 10 | 0.01% |
| 24 | `ucomiss` | 10 | 0.01% |
| 25 | `subl (lock operand)` | 6 | 0.00% |

#### Hand-written vs compiler-emitted — the confusion matrix

Symbol maps were found for **0 of 52** objects in this column. ⛔ **No symbol map: this column is NOT attributed**, and every instruction below sits in the `-` cell rather than being counted as compiler output.

| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 151,688 | 100.00% | 98.9% |

#### What is NOT covered, bucketed by ISA extension

| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 908 | 0.60% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
Symbol maps were found for **0 of 19** objects in this column. ⛔ **No symbol map: this column is NOT attributed**, and every instruction below sits in the `-` cell rather than being counted as compiler output.

| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 112,104 | 100.00% | 88.2% |
...
- **Covered by the model: 652,016 (90.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 69,967 (9.7%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 722,331 | 100.00% | 90.3% |

#### What is NOT covered, bucketed by ISA extension

| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 52,988 | 7.34% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 330,351 (87.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 34,626 (9.2%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `-` | this column has no symbol map | 377,330 | 100.00% | 87.5% |
...
| SSE-legacy (xmm) | 10,628 | 2.82% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,382,616 (91.6%)**.
...
- **Not covered: 401,653 instructions (8.4% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 135,491 | 2.83% | 33.7% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 27.0% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.5% |
...
|---|---|---|---|---|
| `cc1` | must read ~0% hand-written | 0.0% | 0.0% | ✅ both routes as pre-registered |
| `glibc` | must read substantially hand-written | 0.0% | 0.0% | ⛔ NEITHER route is as pre-registered |
