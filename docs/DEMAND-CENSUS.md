...
<!-- census-model: mnemonics=184 sha=0624b00aa469cce0 rules=27142ce74fdadae0 -->
...
The model covers **184 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 557,507 (92.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 45,375 (7.5%).
...
| SSE-legacy (xmm) | 2,943 | 0.49% | `mulpd`, `addpd`, `mulps`, `palignr`, `addps`, `pcmpistri` |
...
- **Covered by the model: 2,986,068 (95.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 114,160 (3.7%).
...
| SSE-legacy (xmm) | 20,320 | 0.65% | `pmaddwd`, `packssdw`, `psubusb`, `pmaxsw`, `pmaddubsw`, `pminsw` |
...
- **Covered by the model: 150,699 (99.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 977 (0.6%).
...
| 7 | `mulps` | 10 | 0.01% |
| 8 | `divps` | 10 | 0.01% |
| 9 | `subl (lock operand)` | 6 | 0.00% |
| 10 | `addl (lock operand)` | 6 | 0.00% |
| 11 | `cvtdq2pd` | 5 | 0.00% |
| 12 | `mulpd` | 5 | 0.00% |
| 13 | `cvttpd2dq` | 4 | 0.00% |
| 14 | `pextrw` | 2 | 0.00% |
...
| SSE-legacy (xmm) | 164 | 0.11% | `pmuludq`, `cmpnleps`, `cmpnlepd`, `cvttps2dq`, `cvtdq2ps`, `mulps` |
...
- **Covered by the model: 655,447 (90.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 66,536 (9.2%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 49,570 | 6.86% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 334,358 (88.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 30,619 (8.1%).
...
| SSE-legacy (xmm) | 6,621 | 1.75% | `psadbw`, `pmaxsw`, `pabsw`, `pmaddwd`, `pmuludq`, `pmaddubsw` |
...
- **4,784,269 instructions**, covered by the model **4,420,469 (92.4%)**.
...
- **Not covered: 363,800 instructions (7.6% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| AVX2/AVX (ymm) | 108,554 | 2.27% | 29.8% |
| SSE-legacy (xmm) | 97,672 | 2.04% | 26.8% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 17.1% |
