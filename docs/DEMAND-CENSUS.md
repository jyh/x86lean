...
<!-- census-model: mnemonics=162 sha=557a5f7646ba8f3f rules=f64d8a3877b19e66 -->
...
The model covers **162 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,377,724 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,117 (0.0%).
...
| 21 | `cvtsi2sdl` | 6 | 0.00% |
| 22 | `pcmpestri` | 6 | 0.00% |
| 23 | `cmpxchgl (lock operand)` | 6 | 0.00% |
| 24 | `minsd` | 5 | 0.00% |
| 25 | `pause` | 4 | 0.00% |
...
| SSE-legacy (xmm) | 519 | 0.01% | `cvtsi2sd`, `mulsd`, `addsd`, `divsd`, `cvtsi2ss`, `cvtss2sd` |
...
- **Covered by the model: 864,233 (98.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,318 (1.7%).
...
| SSE-legacy (xmm) | 1,301 | 0.15% | `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd`, `subss` |
...
- **Covered by the model: 547,313 (90.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 55,569 (9.2%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 69.4% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 13,133 | 2.18% | `mulsd`, `addsd`, `subsd`, `mulss`, `mulpd`, `addpd` |
...
- **Covered by the model: 2,957,752 (94.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 142,476 (4.6%).
...
| `CC` | both routes say compiler-emitted | 2,746,475 | 88.11% | 98.6% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 48,623 | 1.56% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
| 8 | `cvtsi2sd` | 60 | 0.04% |
| 9 | `cvtsd2ss` | 46 | 0.03% |
| 10 | `cvtsi2ssl` | 45 | 0.03% |
| 11 | `cvtss2sd` | 39 | 0.03% |
| 12 | `divsd` | 25 | 0.02% |
| 13 | `cvttsd2si` | 22 | 0.01% |
| 14 | `cmpnleps` | 16 | 0.01% |
| 15 | `cmpnlepd` | 15 | 0.01% |
| 16 | `addsd` | 13 | 0.01% |
| 17 | `cvttps2dq` | 12 | 0.01% |
| 18 | `subsd` | 11 | 0.01% |
| 19 | `cvtdq2ps` | 10 | 0.01% |
| 20 | `mulps` | 10 | 0.01% |
| 21 | `divps` | 10 | 0.01% |
| 22 | `subl (lock operand)` | 6 | 0.00% |
| 23 | `addl (lock operand)` | 6 | 0.00% |
| 24 | `cvtdq2pd` | 5 | 0.00% |
| 25 | `mulpd` | 5 | 0.00% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 851 | 0.56% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 652,524 (90.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 69,459 (9.6%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 52,480 | 7.27% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 330,695 (87.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 34,282 (9.1%).
...
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 98.1% |
...
| SSE-legacy (xmm) | 10,284 | 2.73% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,384,872 (91.7%)**.
...
- **Not covered: 399,397 instructions (8.3% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 133,235 | 2.78% | 33.4% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 27.2% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.6% |
| MMX (mm) | 42,881 | 0.90% | 10.7% |
