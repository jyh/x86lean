...
<!-- census-model: mnemonics=144 sha=b711a06c2fa65a05 rules=203a76ae4de9a9be -->
...
The model covers **144 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,376,967 (99.9%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,874 (0.1%).
...
| 10 | `cvtsi2ss` | 41 | 0.00% |
| 11 | `incsspq` | 28 | 0.00% |
| 12 | `xaddl (lock operand)` | 28 | 0.00% |
| 13 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 14 | `movlpd` | 23 | 0.00% |
| 15 | `addl (lock operand)` | 22 | 0.00% |
| 16 | `cvtss2sd` | 19 | 0.00% |
| 17 | `cvttsd2si` | 18 | 0.00% |
| 18 | `comisd` | 18 | 0.00% |
| 19 | `divss` | 17 | 0.00% |
| 20 | `rdsspq` | 12 | 0.00% |
| 21 | `subsd` | 10 | 0.00% |
...
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 495 | 0.01% | 99.2% |
...
| SSE-legacy (xmm) | 1,276 | 0.02% | `shufpd`, `movhlps`, `cvtsi2sd`, `shufps`, `mulsd`, `addsd` |
...
- **Covered by the model: 863,625 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,926 (1.8%).
...
| SSE-legacy (xmm) | 1,909 | 0.22% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 540,715 (89.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 62,167 (10.3%).
...
| 13 | `vpcmpeqb` | 1,114 | 0.18% |
| 14 | `vpmovmskb` | 1,030 | 0.17% |
| 15 | `vaddsd` | 1,011 | 0.17% |
| 16 | `fld` | 879 | 0.15% |
| 17 | `vmovdqu64` | 821 | 0.14% |
| 18 | `vsubsd` | 819 | 0.14% |
| 19 | `ucomisd` | 726 | 0.12% |
| 20 | `vmovaps` | 699 | 0.12% |
| 21 | `vzeroupper` | 662 | 0.11% |
| 22 | `vpcmpeqd` | 630 | 0.10% |
| 23 | `comisd` | 606 | 0.10% |
| 24 | `kmovd` | 606 | 0.10% |
| 25 | `fmul` | 599 | 0.10% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 68.8% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 13,783 | 2.28% | 65.2% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 66.6% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 93.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 19,731 | 3.27% | `movapd`, `mulsd`, `addsd`, `subsd`, `ucomisd`, `comisd` |
...
- **Covered by the model: 194,555 (64.0%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 109,215 (35.9%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 43.4% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 99.8% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 75.6% |
...
| SSE-legacy (xmm) | 15,479 | 5.09% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `pshufb`, `packssdw`, `pinsrw` |
...
- **Covered by the model: 2,953,923 (94.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 146,305 (4.7%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 48.4% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 133 | 0.00% | 74.4% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 127,628 | 4.09% | 96.0% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 52,452 | 1.68% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,854 (98.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,822 (1.2%).
...
| 14 | `divsd` | 25 | 0.02% |
| 15 | `movapd` | 22 | 0.01% |
| 16 | `cvttsd2si` | 22 | 0.01% |
| 17 | `cmpnleps` | 16 | 0.01% |
| 18 | `cmpnlepd` | 15 | 0.01% |
| 19 | `comisd` | 15 | 0.01% |
| 20 | `unpcklps` | 14 | 0.01% |
| 21 | `addsd` | 13 | 0.01% |
| 22 | `cvttps2dq` | 12 | 0.01% |
| 23 | `subsd` | 11 | 0.01% |
| 24 | `ucomiss` | 10 | 0.01% |
| 25 | `cvtdq2ps` | 10 | 0.01% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 282 | 0.19% | 86.5% |
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 98.8% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 1,001 | 0.66% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 97,958 (87.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 8,715 (7.8%).
...
| 15 | `movlpd` | 256 | 0.23% |
| 16 | `endbr64` | 235 | 0.21% |
| 17 | `pxor (vector operand)` | 224 | 0.20% |
| 18 | `pand (vector operand)` | 192 | 0.17% |
| 19 | `psrlw (vector operand)` | 144 | 0.13% |
| 20 | `movhlps` | 132 | 0.12% |
| 21 | `punpcklwd (vector operand)` | 128 | 0.11% |
| 22 | `punpckhwd (vector operand)` | 128 | 0.11% |
| 23 | `por (vector operand)` | 64 | 0.06% |
| 24 | `emms` | 24 | 0.02% |
| 25 | `sfence` | 10 | 0.01% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 59,515 | 53.09% | 79.8% |
| `CC` | both routes say compiler-emitted | 52,317 | 46.67% | 96.0% |
...
| SSE-legacy (xmm) | 6,902 | 6.16% | `movntdqa`, `paddsw`, `pmulhw`, `movntdq`, `movhpd`, `psubsw` |
...
- **Covered by the model: 649,722 (89.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 72,261 (10.0%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 72.2% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 88.5% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 89.3% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 98.2% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 87.0% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 55,282 | 7.65% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 329,173 (87.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 35,804 (9.5%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 46.4% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 77.3% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 89.6% |
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 97.9% |
...
| SSE-legacy (xmm) | 11,806 | 3.13% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,375,185 (91.4%)**.
...
- **Not covered: 409,084 instructions (8.6% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 142,922 | 2.99% | 34.9% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 26.5% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.2% |
| MMX (mm) | 42,881 | 0.90% | 10.5% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 8.3% |
| CET-IBT | 16,488 | 0.34% | 4.0% |
