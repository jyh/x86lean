...
<!-- census-model: mnemonics=197 sha=79d917e19f58438c rules=a17bdc13f28ac291 -->
...
The model covers **197 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 559,250 (92.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 43,632 (7.2%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 77.3% |
...
| SSE-legacy (xmm) | 1,200 | 0.20% | `palignr`, `pcmpistri`, `pminub`, `movntps`, `movmskps`, `movmskpd` |
...
- **Covered by the model: 150,727 (99.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 949 (0.6%).
...
| SSE-legacy (xmm) | 136 | 0.09% | `pmuludq`, `cmpnleps`, `cmpnlepd`, `cvttps2dq`, `cvtdq2ps`, `cvtdq2pd` |
...
- **4,784,269 instructions**, covered by the model **4,422,176 (92.4%)**.
...
- **Not covered: 362,093 instructions (7.6% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| AVX2/AVX (ymm) | 108,554 | 2.27% | 30.0% |
| SSE-legacy (xmm) | 95,965 | 2.01% | 26.5% |
