...
<!-- census-model: mnemonics=135 sha=756af18bbd2a48a9 rules=2d8eaea351934cb1 -->
...
The model covers **135 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,375,792 (99.9%).**
...
- Not mapped at all, and therefore counted as NOT covered: 4,049 (0.1%).
...
| 12 | `cvtsi2ss` | 41 | 0.00% |
| 13 | `incsspq` | 28 | 0.00% |
| 14 | `xaddl (lock operand)` | 28 | 0.00% |
| 15 | `orps` | 27 | 0.00% |
| 16 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 17 | `movlpd` | 23 | 0.00% |
| 18 | `addl (lock operand)` | 22 | 0.00% |
| 19 | `cvtss2sd` | 19 | 0.00% |
| 20 | `cvttsd2si` | 18 | 0.00% |
| 21 | `comisd` | 18 | 0.00% |
| 22 | `divss` | 17 | 0.00% |
| 23 | `andps` | 16 | 0.00% |
| 24 | `rdsspq` | 12 | 0.00% |
...
|---|---|---|---|
| SSE-legacy (xmm) | 2,451 | 0.05% | `xorps`, `shufpd`, `movhlps`, `cvtsi2sd`, `pandn`, `shufps` |
...
- **Covered by the model: 863,612 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,939 (1.8%).
...
| SSE-legacy (xmm) | 1,922 | 0.22% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 537,834 (89.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 65,048 (10.8%).
...
| 16 | `fld` | 879 | 0.15% |
| 17 | `andpd` | 862 | 0.14% |
| 18 | `vmovdqu64` | 821 | 0.14% |
| 19 | `vsubsd` | 819 | 0.14% |
| 20 | `ucomisd` | 726 | 0.12% |
| 21 | `vmovaps` | 699 | 0.12% |
| 22 | `vzeroupper` | 662 | 0.11% |
| 23 | `andps` | 654 | 0.11% |
| 24 | `vpcmpeqd` | 630 | 0.10% |
| 25 | `comisd` | 606 | 0.10% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 65.1% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 13,783 | 2.28% | 65.2% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 66.1% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 93.1% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 22,612 | 3.75% | `movapd`, `mulsd`, `addsd`, `subsd`, `andpd`, `ucomisd` |
...
- **Covered by the model: 194,144 (63.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 109,626 (36.1%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 43.2% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 99.4% |
...
| SSE-legacy (xmm) | 15,890 | 5.23% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `pshufb`, `packssdw`, `pinsrw` |
...
- **Covered by the model: 2,952,316 (94.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 16,670 (0.53%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 148,000 (4.7%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 48.0% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 54,059 | 1.73% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,739 (98.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,937 (1.3%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 1,116 | 0.74% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 648,488 (89.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 73,495 (10.2%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 72.1% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 87.7% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 89.3% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 98.1% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 86.9% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 56,516 | 7.82% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 328,984 (87.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 35,993 (9.5%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 46.3% |
...
| SSE-legacy (xmm) | 11,995 | 3.18% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,371,629 (91.4%)**.
...
- **Not covered: 412,640 instructions (8.6% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 146,478 | 3.06% | 35.5% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 26.3% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.1% |
| MMX (mm) | 42,881 | 0.90% | 10.4% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 8.2% |
