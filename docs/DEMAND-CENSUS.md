...
<!-- census-model: mnemonics=196 sha=78ba84f1718ef875 rules=69f9ee5ce06f1421 -->
...
The model covers **196 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,378,222 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 1,619 (0.0%).
...
| 2 | `cpuid` | 51 | 0.00% |
| 3 | `incsspq` | 28 | 0.00% |
| 4 | `xaddl (lock operand)` | 28 | 0.00% |
| 5 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 6 | `addl (lock operand)` | 22 | 0.00% |
| 7 | `rdsspq` | 12 | 0.00% |
| 8 | `rdseedl` | 8 | 0.00% |
| 9 | `pmuludq` | 8 | 0.00% |
| 10 | `pcmpestri` | 6 | 0.00% |
| 11 | `cmpxchgl (lock operand)` | 6 | 0.00% |
| 12 | `pause` | 4 | 0.00% |
| 13 | `pextrw` | 4 | 0.00% |
| 14 | `pinsrw` | 3 | 0.00% |
| 15 | `hlt` | 2 | 0.00% |
| 16 | `rdrandl` | 2 | 0.00% |
| 17 | `subl (lock operand)` | 2 | 0.00% |
| 18 | `xgetbv` | 1 | 0.00% |
...
| SSE-legacy (xmm) | 21 | 0.00% | `pmuludq`, `pcmpestri`, `pextrw`, `pinsrw` |
...
- **Covered by the model: 865,527 (98.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 14,024 (1.6%).
...
| SSE-legacy (xmm) | 7 | 0.00% | `movmskpd`, `pinsrw`, `cmpltsd`, `cmpltss` |
| XSAVE | 2 | 0.00% | `xgetbv` |
...
- **Covered by the model: 559,229 (92.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 43,653 (7.2%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 77.2% |
...
| SSE-legacy (xmm) | 1,221 | 0.20% | `palignr`, `pcmpistri`, `pminub`, `movntps`, `movmskps`, `movmskpd` |
...
- **Covered by the model: 195,087 (64.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 108,683 (35.8%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 43.7% |
...
| SSE-legacy (xmm) | 14,947 | 4.92% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `pshufb`, `packssdw`, `pinsrw` |
...
- **Covered by the model: 2,987,538 (95.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 112,690 (3.6%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 49.2% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 133 | 0.00% | 98.5% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 127,628 | 4.09% | 96.2% |
| `CC` | both routes say compiler-emitted | 2,746,475 | 88.11% | 99.7% |
...
| SSE-legacy (xmm) | 18,850 | 0.60% | `pmaddwd`, `packssdw`, `psubusb`, `pmaxsw`, `pmaddubsw`, `pminsw` |
...
- **Covered by the model: 150,726 (99.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 950 (0.6%).
...
| 5 | `cvttps2dq` | 12 | 0.01% |
| 6 | `cvtdq2ps` | 10 | 0.01% |
| 7 | `subl (lock operand)` | 6 | 0.00% |
| 8 | `addl (lock operand)` | 6 | 0.00% |
| 9 | `cvtdq2pd` | 5 | 0.00% |
| 10 | `cvttpd2dq` | 4 | 0.00% |
| 11 | `pextrw` | 2 | 0.00% |
| 12 | `pinsrw` | 2 | 0.00% |
...
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 99.4% |
...
| SSE-legacy (xmm) | 137 | 0.09% | `pmuludq`, `cmpnleps`, `cmpnlepd`, `cvttps2dq`, `cvtdq2ps`, `cvtdq2pd` |
...
- **Covered by the model: 655,525 (90.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 66,458 (9.2%).
...
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 99.3% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 49,492 | 6.85% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 334,441 (88.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 30,536 (8.1%).
...
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 99.5% |
...
| SSE-legacy (xmm) | 6,538 | 1.73% | `psadbw`, `pmaxsw`, `pabsw`, `pmaddwd`, `pmuludq`, `pmaddubsw` |
...
- **4,784,269 instructions**, covered by the model **4,422,175 (92.4%)**.
...
- **Not covered: 362,094 instructions (7.6% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| AVX2/AVX (ymm) | 108,554 | 2.27% | 30.0% |
| SSE-legacy (xmm) | 95,966 | 2.01% | 26.5% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 17.2% |
| MMX (mm) | 43,045 | 0.90% | 11.9% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 9.4% |
| CET-IBT | 16,488 | 0.34% | 4.6% |
