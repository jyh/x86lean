...
<!-- census-model: mnemonics=146 sha=571d5c860979fd10 rules=c68a24b86fef0730 -->
...
The model covers **146 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,376,975 (99.9%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,866 (0.1%).
...
| SSE-legacy (xmm) | 1,268 | 0.02% | `shufpd`, `movhlps`, `cvtsi2sd`, `shufps`, `mulsd`, `addsd` |
...
- **Covered by the model: 863,646 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,905 (1.8%).
...
| SSE-legacy (xmm) | 1,888 | 0.21% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 544,806 (90.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 58,076 (9.6%).
...
| 18 | `ucomisd` | 726 | 0.12% |
| 19 | `vmovaps` | 699 | 0.12% |
| 20 | `vzeroupper` | 662 | 0.11% |
| 21 | `vpcmpeqd` | 630 | 0.10% |
| 22 | `comisd` | 606 | 0.10% |
| 23 | `kmovd` | 606 | 0.10% |
| 24 | `fmul` | 599 | 0.10% |
| 25 | `vmovdqa` | 593 | 0.10% |
...
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 67.9% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 94.1% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 15,640 | 2.59% | `mulsd`, `addsd`, `subsd`, `ucomisd`, `comisd`, `ucomiss` |
...
- **Covered by the model: 2,955,527 (94.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 144,701 (4.6%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 50,848 | 1.63% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,876 (98.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,800 (1.2%).
...
| 13 | `comiss` | 27 | 0.02% |
| 14 | `divsd` | 25 | 0.02% |
| 15 | `cvttsd2si` | 22 | 0.01% |
| 16 | `cmpnleps` | 16 | 0.01% |
| 17 | `cmpnlepd` | 15 | 0.01% |
| 18 | `comisd` | 15 | 0.01% |
| 19 | `unpcklps` | 14 | 0.01% |
| 20 | `addsd` | 13 | 0.01% |
| 21 | `cvttps2dq` | 12 | 0.01% |
| 22 | `subsd` | 11 | 0.01% |
| 23 | `ucomiss` | 10 | 0.01% |
| 24 | `cvtdq2ps` | 10 | 0.01% |
| 25 | `mulps` | 10 | 0.01% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 282 | 0.19% | 92.6% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 979 | 0.65% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 650,340 (90.0%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 71,643 (9.9%).
...
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 98.4% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 54,664 | 7.57% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 329,462 (87.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 35,515 (9.4%).
...
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 98.0% |
...
| SSE-legacy (xmm) | 11,517 | 3.05% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,377,718 (91.5%)**.
...
- **Not covered: 406,551 instructions (8.5% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 140,389 | 2.93% | 34.5% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 26.7% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.3% |
