...
<!-- census-model: mnemonics=124 sha=3e44d58b45c5efd3 rules=f8a28131714dbde8 -->
...
The model covers **124 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,372,102 (99.9%).**
...
- Not mapped at all, and therefore counted as NOT covered: 7,739 (0.1%).
...
| 15 | `cvtsi2ss` | 41 | 0.00% |
| 16 | `incsspq` | 28 | 0.00% |
| 17 | `xaddl (lock operand)` | 28 | 0.00% |
| 18 | `orps` | 27 | 0.00% |
| 19 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 20 | `movlpd` | 23 | 0.00% |
| 21 | `addl (lock operand)` | 22 | 0.00% |
| 22 | `cvtss2sd` | 19 | 0.00% |
| 23 | `cvttsd2si` | 18 | 0.00% |
| 24 | `comisd` | 18 | 0.00% |
| 25 | `divss` | 17 | 0.00% |
...
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 495 | 0.01% | 93.5% |
...
|---|---|---|---|
| SSE-legacy (xmm) | 6,137 | 0.11% | `movhps`, `xorps`, `shufpd`, `movhlps`, `cvtsi2sd`, `pandn` |
...
- **Covered by the model: 863,392 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 16,159 (1.8%).
...
| SSE-legacy (xmm) | 2,142 | 0.24% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 534,533 (88.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 68,349 (11.3%).
...
| 11 | `pcmpeqb` | 1,309 | 0.22% |
| 12 | `fstpt` | 1,142 | 0.19% |
| 13 | `vmulsd` | 1,141 | 0.19% |
| 14 | `pmovmskb` | 1,118 | 0.19% |
| 15 | `vpcmpeqb` | 1,114 | 0.18% |
| 16 | `vpmovmskb` | 1,030 | 0.17% |
| 17 | `vaddsd` | 1,011 | 0.17% |
| 18 | `fld` | 879 | 0.15% |
| 19 | `andpd` | 862 | 0.14% |
| 20 | `vmovdqu64` | 821 | 0.14% |
| 21 | `vsubsd` | 819 | 0.14% |
| 22 | `ucomisd` | 726 | 0.12% |
| 23 | `vmovaps` | 699 | 0.12% |
| 24 | `vzeroupper` | 662 | 0.11% |
| 25 | `andps` | 654 | 0.11% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 59.7% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 13,783 | 2.28% | 63.4% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 63.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 25,781 | 4.27% | `movapd`, `mulsd`, `addsd`, `subsd`, `pcmpeqb`, `pmovmskb` |
...
- **Covered by the model: 191,622 (63.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 112,148 (36.9%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 42.3% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 81.5% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 75.0% |
...
| SSE-legacy (xmm) | 18,412 | 6.06% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `movhps`, `pshufb`, `packssdw` |
...
- **Covered by the model: 2,948,458 (94.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 16,019 (0.51%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 152,509 (4.9%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 46.4% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 57,916 | 1.86% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,646 (98.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 2,030 (1.3%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 1,209 | 0.80% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 642,156 (88.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 79,843 (11.1%).
...
| 10 | `pcmpgtd` | 1,675 | 0.23% |
| 11 | `pcmpeqw` | 1,627 | 0.23% |
| 12 | `pmulhuw` | 1,391 | 0.19% |
| 13 | `paddsw` | 1,351 | 0.19% |
| 14 | `vpaddd` | 1,274 | 0.18% |
| 15 | `vpaddw` | 1,243 | 0.17% |
| 16 | `pandn` | 1,182 | 0.16% |
| 17 | `pmuldq` | 1,058 | 0.15% |
| 18 | `vpsubw` | 947 | 0.13% |
| 19 | `shufps` | 854 | 0.12% |
| 20 | `packuswb` | 752 | 0.10% |
| 21 | `palignr` | 711 | 0.10% |
| 22 | `movhps` | 649 | 0.09% |
| 23 | `vpabsw` | 637 | 0.09% |
| 24 | `movhlps` | 626 | 0.09% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 70.1% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 86.1% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 88.7% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 97.9% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 85.1% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 62,798 | 8.69% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 327,797 (86.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 12,218 (3.24%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 37,315 (9.9%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 45.3% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 73.4% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 88.6% |
...
| SSE-legacy (xmm) | 13,004 | 3.45% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `movhps` |
...
- **4,784,269 instructions**, covered by the model **4,354,479 (91.0%)**.
...
- **Not covered: 429,790 instructions (9.0% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 163,162 | 3.41% | 38.0% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 25.3% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 14.5% |
| MMX (mm) | 42,881 | 0.90% | 10.0% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 7.9% |
| CET-IBT | 16,488 | 0.34% | 3.8% |
