...
<!-- census-model: mnemonics=180 sha=ce1c438109834492 rules=24dcad16b918670f -->
...
The model covers **180 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.

## COMPILER OUTPUT

### cc1 — 2 ELF x86-64 objects

- **5,379,923 instructions** over 261 distinct objdump mnemonics.
- **Covered by the model: 5,378,065 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 1,776 (0.0%).
...
|---|---|---|---|
| 1 | `endbr64` | 1,490 | 0.03% |
| 2 | `cvtsi2sdq` | 112 | 0.00% |
| 3 | `cpuid` | 51 | 0.00% |
| 4 | `cvtsi2ssq` | 33 | 0.00% |
| 5 | `incsspq` | 28 | 0.00% |
| 6 | `xaddl (lock operand)` | 28 | 0.00% |
| 7 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 8 | `addl (lock operand)` | 22 | 0.00% |
| 9 | `rdsspq` | 12 | 0.00% |
| 10 | `cvtsi2ssl` | 11 | 0.00% |
| 11 | `rdseedl` | 8 | 0.00% |
| 12 | `pmuludq` | 8 | 0.00% |
| 13 | `pcmpestri` | 6 | 0.00% |
| 14 | `cmpxchgl (lock operand)` | 6 | 0.00% |
| 15 | `pause` | 4 | 0.00% |
| 16 | `pextrw` | 4 | 0.00% |
| 17 | `pinsrw` | 3 | 0.00% |
| 18 | `hlt` | 2 | 0.00% |
| 19 | `rdrandl` | 2 | 0.00% |
| 20 | `subl (lock operand)` | 2 | 0.00% |
| 21 | `xgetbv` | 1 | 0.00% |
| 22 | `cvtsd2ss` | 1 | 0.00% |
...
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 495 | 0.01% | 100.0% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 136,972 | 2.55% | 100.0% |
| `CC` | both routes say compiler-emitted | 5,215,297 | 96.94% | 100.0% |
...
| SSE-legacy (xmm) | 178 | 0.00% | `cvtsi2sdq`, `cvtsi2ssq`, `cvtsi2ssl`, `pmuludq`, `pcmpestri`, `pextrw` |
...
- **Covered by the model: 865,110 (98.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 14,441 (1.6%).
...
| 2 | `cvtsi2ssq` | 264 | 0.03% |
| 3 | `fstp` | 255 | 0.03% |
| 4 | `fxch` | 196 | 0.02% |
| 5 | `fldcw` | 170 | 0.02% |
| 6 | `cvtsi2sdq` | 153 | 0.02% |
| 7 | `fldt` | 140 | 0.02% |
| 8 | `fstpt` | 134 | 0.02% |
| 9 | `fildll` | 131 | 0.01% |
| 10 | `hlt` | 105 | 0.01% |
...
| `CC` | both routes say compiler-emitted | 863,382 | 98.16% | 98.4% |
...
| x87 (st) | 1,897 | 0.22% | `fstp`, `fxch`, `fldcw`, `fldt`, `fstpt`, `fildll` |
| SSE-legacy (xmm) | 424 | 0.05% | `cvtsi2ssq`, `cvtsi2sdq`, `movmskpd`, `pinsrw`, `cmpltsd`, `cmpltss` |
| VEX-128 (v… xmm) | 152 | 0.02% | `vmovdqa`, `vpxor`, `vpclmulqdq`, `vpshufb`, `vpsrldq`, `vpinsrd` |
| GPR/other (unclassified) | 105 | 0.01% | `hlt` |
...
- Not mapped at all, and therefore counted as NOT covered: 45,620 (7.6%).
...
|---|---|---|---|
| 1 | `fldt` | 2,392 | 0.40% |
| 2 | `vmovups` | 2,321 | 0.38% |
| 3 | `fstp` | 2,291 | 0.38% |
| 4 | `fxch` | 1,516 | 0.25% |
| 5 | `vmovdqu` | 1,338 | 0.22% |
| 6 | `vmovsd` | 1,336 | 0.22% |
| 7 | `fstpt` | 1,142 | 0.19% |
| 8 | `vmulsd` | 1,141 | 0.19% |
| 9 | `vpcmpeqb` | 1,114 | 0.18% |
| 10 | `vpmovmskb` | 1,030 | 0.17% |
| 11 | `vaddsd` | 1,011 | 0.17% |
| 12 | `fld` | 879 | 0.15% |
| 13 | `vmovdqu64` | 821 | 0.14% |
| 14 | `vsubsd` | 819 | 0.14% |
| 15 | `vmovaps` | 699 | 0.12% |
| 16 | `vzeroupper` | 662 | 0.11% |
| 17 | `vpcmpeqd` | 630 | 0.10% |
| 18 | `kmovd` | 606 | 0.10% |
| 19 | `fmul` | 599 | 0.10% |
| 20 | `vmovdqa` | 593 | 0.10% |
| 21 | `vfmadd213pd` | 554 | 0.09% |
| 22 | `syscall` | 544 | 0.09% |
| 23 | `vmovq` | 490 | 0.08% |
| 24 | `faddp` | 474 | 0.08% |
| 25 | `cmpxchgl (lock operand)` | 469 | 0.08% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 69.5% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 13,783 | 2.28% | 65.2% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 73.3% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 96.0% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| x87 (st) | 13,773 | 2.28% | `fldt`, `fstp`, `fxch`, `fstpt`, `fld`, `fmul` |
...
| SSE-legacy (xmm) | 3,188 | 0.53% | `mulpd`, `addpd`, `mulps`, `palignr`, `addps`, `pcmpistri` |
...
| SSE (MXCSR state) | 382 | 0.06% | `stmxcsr`, `ldmxcsr`, `vstmxcsr`, `vldmxcsr` |
| CET-IBT | 85 | 0.01% | `endbr64` |
| BMI1 | 64 | 0.01% | `blsmskl`, `blsmskq`, `blsrl`, `andnl`, `andnq` |
| PREFETCH | 64 | 0.01% | `prefetcht1` |
| GPR/other (unclassified) | 62 | 0.01% | `xtest`, `hlt`, `fxsave`, `fxrstor`, `xsavec`, `rdpkru` |
| CPUID | 52 | 0.01% | `cpuid` |
...
- **Covered by the model: 195,039 (64.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 108,731 (35.8%).
...
| 20 | `vmovq` | 1,385 | 0.46% |
| 21 | `vpsubw` | 1,367 | 0.45% |
| 22 | `pshufb` | 1,347 | 0.44% |
| 23 | `vpaddsw` | 1,238 | 0.41% |
| 24 | `vpackssdw` | 1,214 | 0.40% |
| 25 | `vmovd` | 1,166 | 0.38% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 43.6% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 99.8% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 76.0% |
| `CC` | both routes say compiler-emitted | 93,471 | 30.76% | 99.2% |
...
| SSE-legacy (xmm) | 14,995 | 4.94% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `pshufb`, `packssdw`, `pinsrw` |
...
- **3,116,986 instructions** over 592 distinct objdump mnemonics.
- **Covered by the model: 2,983,754 (95.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 16,758 (0.54%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 116,474 (3.7%).
...
| 5 | `movq (vector operand)` | 5,040 | 0.16% |
| 6 | `vshufps` | 4,340 | 0.14% |
| 7 | `vpaddd` | 4,275 | 0.14% |
| 8 | `vpmaddwd` | 4,034 | 0.13% |
| 9 | `vpaddw` | 3,966 | 0.13% |
| 10 | `vaddps` | 3,418 | 0.11% |
| 11 | `vpsrad` | 2,949 | 0.09% |
| 12 | `vsubps` | 2,791 | 0.09% |
| 13 | `vmulps` | 2,620 | 0.08% |
| 14 | `packssdw` | 2,595 | 0.08% |
| 15 | `vpsubw` | 2,456 | 0.08% |
| 16 | `paddw (vector operand)` | 2,303 | 0.07% |
| 17 | `vmovdqu` | 2,176 | 0.07% |
| 18 | `psubusb` | 1,762 | 0.06% |
| 19 | `movd (vector operand)` | 1,757 | 0.06% |
| 20 | `vpunpcklwd` | 1,633 | 0.05% |
| 21 | `vpunpckhwd` | 1,563 | 0.05% |
| 22 | `vpackssdw` | 1,439 | 0.05% |
| 23 | `vxorps` | 1,367 | 0.04% |
| 24 | `vmovq` | 1,208 | 0.04% |
| 25 | `vpand` | 1,185 | 0.04% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 48.7% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 133 | 0.00% | 74.4% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 127,628 | 4.09% | 96.1% |
| `CC` | both routes say compiler-emitted | 2,746,475 | 88.11% | 99.6% |
...
| SSE-legacy (xmm) | 22,634 | 0.73% | `pmaddwd`, `packssdw`, `psubusb`, `cvtsd2ss`, `pmaxsw`, `cvtsi2ssl` |
| MMX (mm) | 19,887 | 0.64% | `movq`, `paddw`, `movd`, `psubw`, `punpcklbw`, `pxor` |
...
| x87 (st) | 19 | 0.00% | `fstpl`, `fldl`, `fprem`, `fnstsw`, `fstp`, `fildll` |
...
- **151,688 instructions** over 230 distinct objdump mnemonics.
- **Covered by the model: 150,468 (99.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,208 (0.8%).
...
| 11 | `mulps` | 10 | 0.01% |
| 12 | `divps` | 10 | 0.01% |
| 13 | `subl (lock operand)` | 6 | 0.00% |
| 14 | `addl (lock operand)` | 6 | 0.00% |
| 15 | `cvtdq2pd` | 5 | 0.00% |
| 16 | `mulpd` | 5 | 0.00% |
| 17 | `cvttpd2dq` | 4 | 0.00% |
| 18 | `pextrw` | 2 | 0.00% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 282 | 0.19% | 94.0% |
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 99.2% |
...
| SSE-legacy (xmm) | 395 | 0.26% | `cvtsi2ssl`, `pmuludq`, `cvtsd2ss`, `cvtsi2ssq`, `cvtsi2sdq`, `cmpnleps` |
...
- **Covered by the model: 98,858 (88.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 5,431 (4.84%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 7,815 (7.0%).
...
| 8 | `psrlw` | 3,564 | 3.18% | ✅ |
| 9 | `cmp` | 3,504 | 3.13% | ✅ |
| 10 | `punpcklbw` | 3,218 | 2.87% | ✅ |
| 11 | `pshufd` | 2,921 | 2.61% | ✅ |
| 12 | `packuswb` | 2,920 | 2.60% | ✅ |
| 13 | `movq` | 2,873 | 2.56% | ✅ |
| 14 | `psllw` | 2,554 | 2.28% | ✅ |
| 15 | `pand` | 2,240 | 2.00% | ✅ |
...
|---|---|---|---|
| 1 | `paddsw` | 2,016 | 1.80% |
| 2 | `movntdqa` | 1,984 | 1.77% |
| 3 | `movq (vector operand)` | 1,867 | 1.67% |
| 4 | `pmulhw` | 1,728 | 1.54% |
| 5 | `punpcklbw (vector operand)` | 897 | 0.80% |
| 6 | `movntdq` | 690 | 0.62% |
| 7 | `psubsw` | 576 | 0.51% |
| 8 | `packuswb (vector operand)` | 576 | 0.51% |
| 9 | `psllw (vector operand)` | 448 | 0.40% |
| 10 | `movd (vector operand)` | 436 | 0.39% |
| 11 | `punpckhbw (vector operand)` | 327 | 0.29% |
| 12 | `psubusb` | 288 | 0.26% |
| 13 | `pshufb` | 256 | 0.23% |
| 14 | `endbr64` | 235 | 0.21% |
| 15 | `pxor (vector operand)` | 224 | 0.20% |
| 16 | `pand (vector operand)` | 192 | 0.17% |
| 17 | `psrlw (vector operand)` | 144 | 0.13% |
| 18 | `punpcklwd (vector operand)` | 128 | 0.11% |
| 19 | `punpckhwd (vector operand)` | 128 | 0.11% |
| 20 | `por (vector operand)` | 64 | 0.06% |
| 21 | `emms` | 24 | 0.02% |
| 22 | `sfence` | 10 | 0.01% |
| 23 | `mfence` | 8 | 0.01% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 59,515 | 53.09% | 80.9% |
| `CC` | both routes say compiler-emitted | 52,317 | 46.67% | 96.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| MMX (mm) | 6,991 | 6.24% | `movq`, `punpcklbw`, `paddsw`, `pmulhw`, `packuswb`, `psllw` |
| SSE-legacy (xmm) | 6,002 | 5.35% | `movntdqa`, `paddsw`, `pmulhw`, `movntdq`, `psubsw`, `pshufb` |
...
- **722,331 instructions** over 409 distinct objdump mnemonics.
- **Covered by the model: 655,217 (90.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 348 (0.05%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 66,766 (9.2%).
...
| 14 | `pmuldq` | 1,058 | 0.15% |
| 15 | `vpsubw` | 947 | 0.13% |
| 16 | `palignr` | 711 | 0.10% |
| 17 | `vpabsw` | 637 | 0.09% |
| 18 | `pmaxsw` | 574 | 0.08% |
| 19 | `vpmaddwd` | 538 | 0.07% |
| 20 | `psubusb` | 477 | 0.07% |
| 21 | `pmaddubsw` | 463 | 0.06% |
| 22 | `pavgw` | 444 | 0.06% |
| 23 | `pminsw` | 428 | 0.06% |
| 24 | `psubsw` | 399 | 0.06% |
| 25 | `pextrw` | 389 | 0.05% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 72.6% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 89.1% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 89.4% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 99.2% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 87.1% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 49,800 | 6.89% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
| MMX (mm) | 420 | 0.06% | `movq`, `movd`, `paddw`, `punpcklbw`, `paddsw`, `pmullw` |
| AVX (state) | 163 | 0.02% | `vzeroupper` |
| AVX-512 (zmm/k) | 24 | 0.00% | `vmovdqa64`, `vpaddd`, `vpsadbw`, `vpslldq`, `vpord`, `vmovdqu64` |
| CPUID | 12 | 0.00% | `cpuid` |
| XSAVE | 8 | 0.00% | `xgetbv` |
| x87 (st) | 4 | 0.00% | `fldcw`, `wait`, `fnstcw` |
| PREFETCH | 4 | 0.00% | `prefetcht2` |
| SSE2 (pause) | 3 | 0.00% | `pause` |

### x264 — 1 ELF x86-64 objects

- **377,330 instructions** over 522 distinct objdump mnemonics.
- **Covered by the model: 333,709 (88.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 12,353 (3.27%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 31,268 (8.3%).
...
| 19 | `vpsadbw` | 504 | 0.13% |
| 20 | `pshufw` | 437 | 0.12% |
| 21 | `vpunpckhqdq` | 421 | 0.11% |
| 22 | `vpmaddubsw` | 414 | 0.11% |
| 23 | `punpcklwd (vector operand)` | 384 | 0.10% |
| 24 | `punpcklbw (vector operand)` | 378 | 0.10% |
| 25 | `pmaddubsw` | 365 | 0.10% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 47.5% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 77.6% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 90.0% |
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 99.2% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| MMX (mm) | 15,747 | 4.17% | `movq`, `paddw`, `psubw`, `movd`, `pxor`, `pmaxsw` |
| VEX-128 (v… xmm) | 13,469 | 3.57% | `vmovdqa`, `vpaddw`, `vpsubw`, `vpaddd`, `vmovd`, `vmovq` |
| SSE-legacy (xmm) | 7,270 | 1.93% | `psadbw`, `pmaxsw`, `pabsw`, `pmaddwd`, `pmuludq`, `pmaddubsw` |
...
- **4,784,269 instructions**, covered by the model **4,417,045 (92.3%)**.
...
- **Not covered: 367,224 instructions (7.7% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| AVX2/AVX (ymm) | 108,554 | 2.27% | 29.6% |
| SSE-legacy (xmm) | 101,096 | 2.11% | 27.5% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 17.0% |
| MMX (mm) | 43,045 | 0.90% | 11.7% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 9.2% |
| CET-IBT | 16,488 | 0.34% | 4.5% |
...
- **Covered by the model: 2,767,164 (99.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 5,711 (0.2%).
...
| GPR/other (unclassified) | 4,899 | 0.18% | `int3`, `ud1l`, `pushfq`, `sti`, `cli`, `outb` |
...
| x87 (st) | 18 | 0.00% | `fninit`, `wait`, `fldl`, `fnclex`, `fildl`, `fnstsw` |
...
| PREFETCH | 2 | 0.00% | `prefetcht1` |
| RDSEED | 2 | 0.00% | `rdseedq` |
| SSE (MXCSR state) | 1 | 0.00% | `ldmxcsr` |
| MMX (mm) | 1 | 0.00% | `emms` |
