...
<!-- census-model: mnemonics=107 sha=b7d01da51d132dce rules=f0ee5b170fc62939 -->
...
The model covers **107 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,315,363 (98.8%).**
...
- Not mapped at all, and therefore counted as NOT covered: 64,478 (1.2%).
...
| 2 | `movups` | 20,774 | 0.39% |
| 3 | `movhps` | 3,493 | 0.06% |
| 4 | `endbr64` | 1,490 | 0.03% |
| 5 | `xorps` | 988 | 0.02% |
| 6 | `pshufd` | 767 | 0.01% |
| 7 | `shufpd` | 323 | 0.01% |
| 8 | `psrldq` | 323 | 0.01% |
...
| 20 | `cvtsi2ss` | 41 | 0.00% |
| 21 | `incsspq` | 28 | 0.00% |
| 22 | `xaddl (lock operand)` | 28 | 0.00% |
| 23 | `orps` | 27 | 0.00% |
| 24 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 25 | `movlpd` | 23 | 0.00% |
...
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 495 | 0.01% | 69.3% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 136,972 | 2.55% | 99.0% |
| `CC` | both routes say compiler-emitted | 5,215,297 | 96.94% | 98.8% |
...
|---|---|---|---|
| SSE-legacy (xmm) | 62,876 | 1.17% | `movaps`, `movups`, `movhps`, `xorps`, `pshufd`, `shufpd` |
...
- **Covered by the model: 853,814 (97.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 25,737 (2.9%).
...
| 7 | `fstp` | 255 | 0.03% |
| 8 | `fxch` | 196 | 0.02% |
| 9 | `cvttss2si` | 176 | 0.02% |
| 10 | `mulss` | 176 | 0.02% |
| 11 | `fldcw` | 170 | 0.02% |
| 12 | `addss` | 154 | 0.02% |
| 13 | `cvtsi2sd` | 151 | 0.02% |
| 14 | `fldt` | 140 | 0.02% |
| 15 | `fstpt` | 134 | 0.02% |
| 16 | `fildll` | 131 | 0.01% |
| 17 | `psrldq` | 116 | 0.01% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 344 | 0.04% | 71.2% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 1,155 | 0.13% | 68.6% |
| `CC` | both routes say compiler-emitted | 863,382 | 98.16% | 97.1% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 11,720 | 1.33% | `movaps`, `movups`, `comiss`, `movss`, `cvtsi2ss`, `cvttss2si` |
...
- **Covered by the model: 516,919 (85.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 85,963 (14.2%).
...
| 5 | `addsd` | 3,079 | 0.51% |
| 6 | `fldt` | 2,392 | 0.40% |
| 7 | `vmovups` | 2,321 | 0.38% |
| 8 | `fstp` | 2,291 | 0.38% |
| 9 | `movups` | 2,111 | 0.35% |
| 10 | `movss` | 2,094 | 0.35% |
| 11 | `subsd` | 1,868 | 0.31% |
| 12 | `fxch` | 1,516 | 0.25% |
| 13 | `vmovdqu` | 1,338 | 0.22% |
| 14 | `vmovsd` | 1,336 | 0.22% |
| 15 | `pcmpeqb` | 1,309 | 0.22% |
| 16 | `fstpt` | 1,142 | 0.19% |
| 17 | `vmulsd` | 1,141 | 0.19% |
| 18 | `pmovmskb` | 1,118 | 0.19% |
| 19 | `vpcmpeqb` | 1,114 | 0.18% |
| 20 | `vpmovmskb` | 1,030 | 0.17% |
| 21 | `vaddsd` | 1,011 | 0.17% |
| 22 | `fld` | 879 | 0.15% |
| 23 | `andpd` | 862 | 0.14% |
| 24 | `vmovdqu64` | 821 | 0.14% |
| 25 | `vsubsd` | 819 | 0.14% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 48.3% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 13,783 | 2.28% | 57.5% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 59.8% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 90.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 43,395 | 7.19% | `movsd`, `movaps`, `movapd`, `mulsd`, `addsd`, `movups` |
...
- **Covered by the model: 184,255 (60.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 119,515 (39.3%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 39.5% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 55.9% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 72.3% |
| `CC` | both routes say compiler-emitted | 93,471 | 30.76% | 98.3% |
...
| SSE-legacy (xmm) | 25,779 | 8.48% | `pmaddwd`, `pmulhrsw`, `psrad`, `pmaddubsw`, `movhps`, `pshufb` |
...
- **Covered by the model: 2,905,936 (93.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 196,660 (6.3%).
...
|---|---|---|---|
| 1 | `movss` | 14,667 | 0.47% |
| 2 | `endbr64` | 11,236 | 0.36% |
| 3 | `vmovaps` | 7,522 | 0.24% |
| 4 | `pmaddwd` | 6,603 | 0.21% |
| 5 | `movaps` | 6,482 | 0.21% |
| 6 | `vmovdqa` | 6,036 | 0.19% |
| 7 | `movups` | 5,852 | 0.19% |
| 8 | `mulss` | 5,229 | 0.17% |
| 9 | `movsd` | 5,206 | 0.17% |
| 10 | `movq (vector operand)` | 5,040 | 0.16% |
| 11 | `psrad` | 4,904 | 0.16% |
| 12 | `addss` | 4,384 | 0.14% |
| 13 | `vshufps` | 4,340 | 0.14% |
| 14 | `vpaddd` | 4,275 | 0.14% |
| 15 | `mulsd` | 4,181 | 0.13% |
| 16 | `vpmaddwd` | 4,034 | 0.13% |
| 17 | `vpaddw` | 3,966 | 0.13% |
| 18 | `addsd` | 3,468 | 0.11% |
| 19 | `vaddps` | 3,418 | 0.11% |
| 20 | `vpsrad` | 2,949 | 0.09% |
| 21 | `vsubps` | 2,791 | 0.09% |
| 22 | `vmulps` | 2,620 | 0.08% |
| 23 | `cvtss2sd` | 2,607 | 0.08% |
| 24 | `packssdw` | 2,595 | 0.08% |
| 25 | `vpsubw` | 2,456 | 0.08% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 41.6% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 133 | 0.00% | 47.4% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 127,628 | 4.09% | 95.3% |
| `CC` | both routes say compiler-emitted | 2,746,475 | 88.11% | 97.4% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 100,438 | 3.22% | `movss`, `movaps`, `pmaddwd`, `movups`, `mulss`, `movsd` |
...
- **Covered by the model: 147,504 (97.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 4,172 (2.8%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 282 | 0.19% | 45.7% |
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 97.3% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 3,351 | 2.21% | `movups`, `movaps`, `movss`, `pshufd`, `cvtsi2ss`, `addss` |
...
- **Covered by the model: 84,705 (75.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 23,136 (20.6%).
...
| 12 | `psubsw` | 576 | 0.51% |
| 13 | `movhpd` | 512 | 0.46% |
| 14 | `movd (vector operand)` | 436 | 0.39% |
| 15 | `punpckhbw (vector operand)` | 327 | 0.29% |
| 16 | `psubusb` | 288 | 0.26% |
| 17 | `pshufb` | 256 | 0.23% |
| 18 | `movlpd` | 256 | 0.23% |
| 19 | `prefetchnta` | 237 | 0.21% |
| 20 | `endbr64` | 235 | 0.21% |
| 21 | `pxor (vector operand)` | 224 | 0.20% |
| 22 | `pand (vector operand)` | 192 | 0.17% |
| 23 | `movaps` | 181 | 0.16% |
| 24 | `movhlps` | 132 | 0.12% |
| 25 | `punpcklwd (vector operand)` | 128 | 0.11% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 59,515 | 53.09% | 65.1% |
| `CC` | both routes say compiler-emitted | 52,317 | 46.67% | 87.3% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 19,918 | 17.77% | `psrlw`, `pshufd`, `packuswb`, `psllw`, `movntdqa`, `paddsw` |
...
- **Covered by the model: 607,471 (84.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 114,544 (15.9%).
...
| 2 | `movaps` | 11,593 | 1.60% |
| 3 | `pmaddwd` | 11,258 | 1.56% |
| 4 | `psrad` | 5,426 | 0.75% |
| 5 | `psadbw` | 4,426 | 0.61% |
| 6 | `movups` | 4,279 | 0.59% |
| 7 | `pshufd` | 3,866 | 0.54% |
| 8 | `pmullw` | 2,727 | 0.38% |
| 9 | `endbr64` | 2,345 | 0.32% |
| 10 | `pmuludq` | 2,332 | 0.32% |
| 11 | `psrldq` | 2,291 | 0.32% |
| 12 | `movsd` | 2,229 | 0.31% |
| 13 | `packssdw` | 1,861 | 0.26% |
| 14 | `vmovdqa` | 1,822 | 0.25% |
| 15 | `vmovdqu` | 1,729 | 0.24% |
| 16 | `pcmpgtd` | 1,675 | 0.23% |
| 17 | `pcmpeqw` | 1,627 | 0.23% |
| 18 | `psraw` | 1,577 | 0.22% |
| 19 | `pmulhuw` | 1,391 | 0.19% |
| 20 | `paddsw` | 1,351 | 0.19% |
| 21 | `vpaddd` | 1,274 | 0.18% |
| 22 | `vpaddw` | 1,243 | 0.17% |
| 23 | `pandn` | 1,182 | 0.16% |
| 24 | `pmuldq` | 1,058 | 0.15% |
| 25 | `vpsubw` | 947 | 0.13% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 61.9% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 75.5% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 87.3% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 95.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 97,483 | 13.50% | `psubusw`, `movaps`, `pmaddwd`, `psrad`, `psadbw`, `movups` |
...
- **Covered by the model: 319,822 (84.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 45,875 (12.2%).
...
| 2 | `movaps` | 2,063 | 0.55% |
| 3 | `vmovdqa` | 1,858 | 0.49% |
| 4 | `paddw (vector operand)` | 1,822 | 0.48% |
| 5 | `vpaddw` | 1,815 | 0.48% |
| 6 | `psubw (vector operand)` | 1,768 | 0.47% |
| 7 | `psadbw` | 1,719 | 0.46% |
| 8 | `movsd` | 1,668 | 0.44% |
| 9 | `pmaxsw` | 1,562 | 0.41% |
| 10 | `vpsubw` | 1,464 | 0.39% |
| 11 | `endbr64` | 1,168 | 0.31% |
| 12 | `movd (vector operand)` | 1,149 | 0.30% |
| 13 | `vpaddd` | 953 | 0.25% |
| 14 | `movss` | 888 | 0.24% |
| 15 | `pxor (vector operand)` | 876 | 0.23% |
| 16 | `pabsw` | 822 | 0.22% |
| 17 | `pmaddwd` | 743 | 0.20% |
| 18 | `vpabsw` | 677 | 0.18% |
| 19 | `vmovd` | 660 | 0.17% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 39.8% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 59.8% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 87.2% |
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 96.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 20,979 | 5.56% | `movaps`, `movsd`, `psadbw`, `movss`, `pmaxsw`, `pabsw` |
...
- **4,784,269 instructions**, covered by the model **4,249,693 (88.8%)**.
...
- **Not covered: 534,576 instructions (11.2% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 267,948 | 5.60% | 50.1% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 20.3% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 11.6% |
| MMX (mm) | 42,881 | 0.90% | 8.0% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 6.3% |
| CET-IBT | 16,488 | 0.34% | 3.1% |
| AVX (state) | 1,241 | 0.03% | 0.2% |
