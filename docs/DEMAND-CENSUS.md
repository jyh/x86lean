...
<!-- census-model: mnemonics=121 sha=78fd97279b737846 rules=4c85ca0ced6bda0b -->
...
The model covers **121 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,371,318 (99.8%).**
...
- Not mapped at all, and therefore counted as NOT covered: 8,523 (0.2%).
...
| 16 | `cvtsi2ss` | 41 | 0.00% |
| 17 | `incsspq` | 28 | 0.00% |
| 18 | `xaddl (lock operand)` | 28 | 0.00% |
| 19 | `orps` | 27 | 0.00% |
| 20 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 21 | `movlpd` | 23 | 0.00% |
| 22 | `addl (lock operand)` | 22 | 0.00% |
| 23 | `cvtss2sd` | 19 | 0.00% |
| 24 | `cvttsd2si` | 18 | 0.00% |
| 25 | `comisd` | 18 | 0.00% |
...
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 495 | 0.01% | 91.9% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 136,972 | 2.55% | 100.0% |
| `CC` | both routes say compiler-emitted | 5,215,297 | 96.94% | 99.8% |
...
|---|---|---|---|
| SSE-legacy (xmm) | 6,921 | 0.13% | `movhps`, `xorps`, `pshufd`, `shufpd`, `movhlps`, `cvtsi2sd` |
...
- **Covered by the model: 863,330 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 16,221 (1.8%).
...
| SSE-legacy (xmm) | 2,204 | 0.25% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 534,369 (88.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 68,513 (11.4%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 59.2% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 25,945 | 4.30% | `movapd`, `mulsd`, `addsd`, `subsd`, `pcmpeqb`, `pmovmskb` |
...
- **Covered by the model: 189,998 (62.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 113,772 (37.4%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 41.6% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 81.0% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 74.0% |
| `CC` | both routes say compiler-emitted | 93,471 | 30.76% | 99.0% |
...
| SSE-legacy (xmm) | 20,036 | 6.59% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `movhps`, `pshufb`, `pshufd` |
...
- **Covered by the model: 2,946,714 (94.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 154,253 (4.9%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 45.7% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 133 | 0.00% | 72.9% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 127,628 | 4.09% | 95.7% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 59,660 | 1.91% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,454 (98.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 2,222 (1.5%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 282 | 0.19% | 74.8% |
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 98.6% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 1,401 | 0.92% | `pshufd`, `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq` |
...
- **Covered by the model: 91,879 (82.0%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,370 (13.7%).
...
| 2 | `pshufd` | 2,921 | 2.61% |
| 3 | `paddsw` | 2,016 | 1.80% |
| 4 | `movntdqa` | 1,984 | 1.77% |
| 5 | `movq (vector operand)` | 1,867 | 1.67% |
| 6 | `pmulhw` | 1,728 | 1.54% |
| 7 | `punpcklbw (vector operand)` | 897 | 0.80% |
| 8 | `movntdq` | 690 | 0.62% |
| 9 | `psubsw` | 576 | 0.51% |
| 10 | `movhpd` | 512 | 0.46% |
| 11 | `psllw (vector operand)` | 448 | 0.40% |
| 12 | `movd (vector operand)` | 436 | 0.39% |
| 13 | `punpckhbw (vector operand)` | 327 | 0.29% |
| 14 | `psubusb` | 288 | 0.26% |
| 15 | `pshufb` | 256 | 0.23% |
| 16 | `movlpd` | 256 | 0.23% |
| 17 | `prefetchnta` | 237 | 0.21% |
| 18 | `endbr64` | 235 | 0.21% |
| 19 | `pxor (vector operand)` | 224 | 0.20% |
| 20 | `pand (vector operand)` | 192 | 0.17% |
| 21 | `psrlw (vector operand)` | 144 | 0.13% |
| 22 | `movhlps` | 132 | 0.12% |
| 23 | `punpcklwd (vector operand)` | 128 | 0.11% |
| 24 | `punpckhwd (vector operand)` | 128 | 0.11% |
| 25 | `por (vector operand)` | 64 | 0.06% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 59,515 | 53.09% | 73.7% |
| `CC` | both routes say compiler-emitted | 52,317 | 46.67% | 91.3% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 12,744 | 11.37% | `pshufd`, `packuswb`, `movntdqa`, `paddsw`, `pmulhw`, `movntdq` |
...
- **Covered by the model: 637,390 (88.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 84,609 (11.7%).
...
| 4 | `pshufd` | 3,866 | 0.54% |
| 5 | `pmullw` | 2,727 | 0.38% |
| 6 | `endbr64` | 2,345 | 0.32% |
| 7 | `pmuludq` | 2,332 | 0.32% |
| 8 | `packssdw` | 1,861 | 0.26% |
| 9 | `vmovdqa` | 1,822 | 0.25% |
| 10 | `vmovdqu` | 1,729 | 0.24% |
| 11 | `pcmpgtd` | 1,675 | 0.23% |
| 12 | `pcmpeqw` | 1,627 | 0.23% |
| 13 | `pmulhuw` | 1,391 | 0.19% |
| 14 | `paddsw` | 1,351 | 0.19% |
| 15 | `vpaddd` | 1,274 | 0.18% |
| 16 | `vpaddw` | 1,243 | 0.17% |
| 17 | `pandn` | 1,182 | 0.16% |
| 18 | `pmuldq` | 1,058 | 0.15% |
| 19 | `vpsubw` | 947 | 0.13% |
| 20 | `shufps` | 854 | 0.12% |
| 21 | `pshuflw` | 752 | 0.10% |
| 22 | `packuswb` | 752 | 0.10% |
| 23 | `palignr` | 711 | 0.10% |
| 24 | `movhps` | 649 | 0.09% |
| 25 | `vpabsw` | 637 | 0.09% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 69.1% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 84.7% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 88.3% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 97.6% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 82.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 67,564 | 9.35% | `psubusw`, `pmaddwd`, `psadbw`, `pshufd`, `pmullw`, `pmuludq` |
...
- **Covered by the model: 326,980 (86.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 38,132 (10.1%).
...
| 22 | `pshufw` | 437 | 0.12% |
| 23 | `pshuflw` | 430 | 0.11% |
| 24 | `vpunpckhqdq` | 421 | 0.11% |
| 25 | `vpmaddubsw` | 414 | 0.11% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 44.4% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 72.7% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 88.1% |
...
| SSE-legacy (xmm) | 13,821 | 3.66% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `movhps` |
...
- **4,784,269 instructions**, covered by the model **4,342,415 (90.8%)**.
...
- **Not covered: 441,854 instructions (9.2% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 175,226 | 3.66% | 39.7% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 24.6% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 14.1% |
| MMX (mm) | 42,881 | 0.90% | 9.7% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 7.7% |
| CET-IBT | 16,488 | 0.34% | 3.7% |
...
- **Covered by the model: 2,767,114 (99.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 5,761 (0.2%).
...
| SSE-legacy (xmm) | 10 | 0.00% | `pshufd`, `pshufb` |
