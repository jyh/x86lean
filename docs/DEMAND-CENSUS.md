...
<!-- census-model: mnemonics=192 sha=e237bcd6cdd3959a rules=2d217dae15df3652 -->
...
The model covers **192 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 559,147 (92.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 43,735 (7.2%).
...
| SSE-legacy (xmm) | 1,303 | 0.22% | `palignr`, `pcmpistri`, `pminub`, `movntps`, `movmskps`, `movmskpd` |
...
- **Covered by the model: 2,987,109 (95.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 113,119 (3.6%).
...
| SSE-legacy (xmm) | 19,279 | 0.62% | `pmaddwd`, `packssdw`, `psubusb`, `pmaxsw`, `pmaddubsw`, `pminsw` |
...
- **Covered by the model: 150,725 (99.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 951 (0.6%).
...
| SSE-legacy (xmm) | 138 | 0.09% | `pmuludq`, `cmpnleps`, `cmpnlepd`, `cvttps2dq`, `cvtdq2ps`, `cvtdq2pd` |
...
- **Covered by the model: 655,512 (90.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 66,471 (9.2%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 49,505 | 6.85% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 334,403 (88.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 30,574 (8.1%).
...
| SSE-legacy (xmm) | 6,576 | 1.74% | `psadbw`, `pmaxsw`, `pabsw`, `pmaddwd`, `pmuludq`, `pmaddubsw` |
...
- **4,784,269 instructions**, covered by the model **4,421,694 (92.4%)**.
...
- **Not covered: 362,575 instructions (7.6% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| AVX2/AVX (ymm) | 108,554 | 2.27% | 29.9% |
| SSE-legacy (xmm) | 96,447 | 2.02% | 26.6% |
