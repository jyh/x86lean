...
<!-- census-model: mnemonics=172 sha=8fdcc306a6506623 rules=08c61b79e0c575f6 -->
...
The model covers **172 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,377,821 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,020 (0.0%).
...
| 19 | `pmuludq` | 8 | 0.00% |
| 20 | `pcmpestri` | 6 | 0.00% |
| 21 | `cmpxchgl (lock operand)` | 6 | 0.00% |
| 22 | `pause` | 4 | 0.00% |
| 23 | `pextrw` | 4 | 0.00% |
| 24 | `pinsrw` | 3 | 0.00% |
| 25 | `hlt` | 2 | 0.00% |
...
| SSE-legacy (xmm) | 422 | 0.01% | `cvtsi2sdq`, `mulsd`, `addsd`, `divsd`, `cvtsi2ssq`, `divss` |
...
- **Covered by the model: 864,431 (98.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,120 (1.7%).
...
| SSE-legacy (xmm) | 1,103 | 0.13% | `cvtsi2ssq`, `mulss`, `addss`, `cvtsi2sdq`, `subss`, `mulsd` |
...
- **Covered by the model: 547,588 (90.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 55,294 (9.2%).
...
| SSE-legacy (xmm) | 12,862 | 2.13% | `mulsd`, `addsd`, `subsd`, `mulss`, `mulpd`, `addpd` |
...
- **Covered by the model: 2,962,128 (95.0%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 138,100 (4.4%).
...
| SSE-legacy (xmm) | 44,260 | 1.42% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `packssdw` |
...
- **Covered by the model: 150,188 (99.0%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,488 (1.0%).
...
| 12 | `cmpnlepd` | 15 | 0.01% |
| 13 | `addsd` | 13 | 0.01% |
| 14 | `cvttps2dq` | 12 | 0.01% |
| 15 | `subsd` | 11 | 0.01% |
| 16 | `cvtdq2ps` | 10 | 0.01% |
| 17 | `mulps` | 10 | 0.01% |
| 18 | `divps` | 10 | 0.01% |
| 19 | `subl (lock operand)` | 6 | 0.00% |
| 20 | `addl (lock operand)` | 6 | 0.00% |
| 21 | `cvtdq2pd` | 5 | 0.00% |
| 22 | `mulpd` | 5 | 0.00% |
| 23 | `cvttpd2dq` | 4 | 0.00% |
| 24 | `divss` | 3 | 0.00% |
...
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 99.0% |
...
| SSE-legacy (xmm) | 675 | 0.44% | `cvtsi2ssl`, `addss`, `mulss`, `pmuludq`, `mulsd`, `cvtsd2ss` |
...
- **Covered by the model: 653,287 (90.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 68,696 (9.5%).
...
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 98.8% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 51,730 | 7.16% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 331,613 (87.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 33,364 (8.8%).
...
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 98.5% |
...
| SSE-legacy (xmm) | 9,366 | 2.48% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,391,113 (91.8%)**.
...
- **Not covered: 393,156 instructions (8.2% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 127,028 | 2.66% | 32.3% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 27.6% |
