...
<!-- census-model: mnemonics=131 sha=6d5c44858342796c rules=c6278134749d11c1 -->
...
The model covers **131 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,372,294 (99.9%).**
...
- Not mapped at all, and therefore counted as NOT covered: 7,547 (0.1%).
...
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 495 | 0.01% | 97.8% |
...
|---|---|---|---|
| SSE-legacy (xmm) | 5,945 | 0.11% | `movhps`, `xorps`, `shufpd`, `movhlps`, `cvtsi2sd`, `pandn` |
...
- **Covered by the model: 863,546 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 16,005 (1.8%).
...
| SSE-legacy (xmm) | 1,988 | 0.23% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 192,716 (63.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 111,054 (36.6%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 42.8% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 81.8% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 75.5% |
...
| SSE-legacy (xmm) | 17,318 | 5.70% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `movhps`, `pshufb`, `packssdw` |
...
- **Covered by the model: 2,951,157 (94.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 16,655 (0.53%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 149,174 (4.8%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 47.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 55,217 | 1.77% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,718 (98.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,958 (1.3%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 1,137 | 0.75% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 97,720 (87.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 8,953 (8.0%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 59,515 | 53.09% | 79.6% |
| `CC` | both routes say compiler-emitted | 52,317 | 46.67% | 95.7% |
...
| SSE-legacy (xmm) | 6,903 | 6.16% | `movntdqa`, `paddsw`, `pmulhw`, `movntdq`, `movhpd`, `psubsw` |
...
- **Covered by the model: 647,472 (89.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 74,511 (10.3%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 71.8% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 87.7% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 89.0% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 98.1% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 86.3% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 57,482 | 7.96% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 328,255 (87.0%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 12,328 (3.27%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 36,747 (9.7%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 45.7% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 75.3% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 89.0% |
...
| SSE-legacy (xmm) | 12,546 | 3.32% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `movhps` |
...
- **4,784,269 instructions**, covered by the model **4,367,038 (91.3%)**.
...
- **Not covered: 417,231 instructions (8.7% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 150,603 | 3.15% | 36.1% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 26.0% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 14.9% |
| MMX (mm) | 42,881 | 0.90% | 10.3% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 8.1% |
