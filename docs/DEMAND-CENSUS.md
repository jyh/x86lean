...
<!-- census-model: mnemonics=152 sha=cb66ce8fdb8c19f3 rules=135184a1588ebb5d -->
...
The model covers **152 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,377,289 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,552 (0.0%).
...
| 9 | `cvtsi2ss` | 41 | 0.00% |
| 10 | `incsspq` | 28 | 0.00% |
| 11 | `xaddl (lock operand)` | 28 | 0.00% |
| 12 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 13 | `addl (lock operand)` | 22 | 0.00% |
| 14 | `cvtss2sd` | 19 | 0.00% |
| 15 | `cvttsd2si` | 18 | 0.00% |
| 16 | `comisd` | 18 | 0.00% |
| 17 | `divss` | 17 | 0.00% |
| 18 | `rdsspq` | 12 | 0.00% |
| 19 | `subsd` | 10 | 0.00% |
...
| SSE-legacy (xmm) | 954 | 0.02% | `shufpd`, `cvtsi2sd`, `shufps`, `mulsd`, `addsd`, `divsd` |
...
- **Covered by the model: 863,665 (98.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,886 (1.8%).
...
| SSE-legacy (xmm) | 1,869 | 0.21% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 544,916 (90.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 57,966 (9.6%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 69.2% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 15,530 | 2.57% | `mulsd`, `addsd`, `subsd`, `ucomisd`, `comisd`, `ucomiss` |
...
- **Covered by the model: 194,847 (64.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 108,923 (35.8%).
...
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 75.7% |
...
| SSE-legacy (xmm) | 15,187 | 5.00% | `pmaddwd`, `pmulhrsw`, `pmaddubsw`, `pshufb`, `packssdw`, `pinsrw` |
...
- **Covered by the model: 2,955,926 (94.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 144,302 (4.6%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 48.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 50,449 | 1.62% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 149,883 (98.8%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,793 (1.2%).
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 972 | 0.64% | `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 651,088 (90.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 70,895 (9.8%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 72.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 53,916 | 7.46% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 330,234 (87.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 34,743 (9.2%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 47.3% |
...
| SSE-legacy (xmm) | 10,745 | 2.85% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,380,836 (91.6%)**.
...
- **Not covered: 403,433 instructions (8.4% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 137,271 | 2.87% | 34.0% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 26.9% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.4% |
| MMX (mm) | 42,881 | 0.90% | 10.6% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 8.4% |
