...
<!-- census-model: mnemonics=168 sha=d4bb32a5d40890fa rules=aeb157e83a489b4a -->
...
The model covers **168 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,377,730 (100.0%).**
...
- Not mapped at all, and therefore counted as NOT covered: 2,111 (0.0%).
...
| 8 | `cvtsi2ssq` | 33 | 0.00% |
| 9 | `incsspq` | 28 | 0.00% |
| 10 | `xaddl (lock operand)` | 28 | 0.00% |
| 11 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 12 | `addl (lock operand)` | 22 | 0.00% |
| 13 | `cvtss2sd` | 19 | 0.00% |
| 14 | `cvttsd2si` | 18 | 0.00% |
| 15 | `divss` | 17 | 0.00% |
| 16 | `rdsspq` | 12 | 0.00% |
| 17 | `cvtsi2ssl` | 11 | 0.00% |
| 18 | `subsd` | 10 | 0.00% |
| 19 | `addss` | 10 | 0.00% |
| 20 | `rdseedl` | 8 | 0.00% |
| 21 | `mulss` | 8 | 0.00% |
| 22 | `pmuludq` | 8 | 0.00% |
| 23 | `pcmpestri` | 6 | 0.00% |
| 24 | `cmpxchgl (lock operand)` | 6 | 0.00% |
| 25 | `pause` | 4 | 0.00% |
...
| SSE-legacy (xmm) | 513 | 0.01% | `cvtsi2sdq`, `mulsd`, `addsd`, `divsd`, `cvtsi2sdl`, `cvtsi2ssq` |
...
- **Covered by the model: 864,234 (98.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 15,317 (1.7%).
...
| SSE-legacy (xmm) | 1,300 | 0.15% | `cvtsi2ssq`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sdq`, `subss` |
...
- **Covered by the model: 547,335 (90.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 55,547 (9.2%).
...
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 68.3% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 94.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 13,111 | 2.17% | `mulsd`, `addsd`, `subsd`, `mulss`, `mulpd`, `addpd` |
...
- **Covered by the model: 2,958,255 (94.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 141,973 (4.6%).
...
| `CC` | both routes say compiler-emitted | 2,746,475 | 88.11% | 98.7% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 48,120 | 1.54% | `pmaddwd`, `mulss`, `addss`, `mulsd`, `addsd`, `cvtss2sd` |
...
- **Covered by the model: 150,004 (98.9%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 1,672 (1.1%).
...
| 9 | `cvtsi2ssq` | 44 | 0.03% |
| 10 | `cvtss2sd` | 39 | 0.03% |
| 11 | `cvtsi2sdl` | 38 | 0.03% |
| 12 | `cvtsi2sdq` | 28 | 0.02% |
| 13 | `divsd` | 25 | 0.02% |
| 14 | `cvttsd2si` | 22 | 0.01% |
| 15 | `cmpnleps` | 16 | 0.01% |
| 16 | `cmpnlepd` | 15 | 0.01% |
| 17 | `addsd` | 13 | 0.01% |
| 18 | `cvttps2dq` | 12 | 0.01% |
| 19 | `subsd` | 11 | 0.01% |
| 20 | `cvtdq2ps` | 10 | 0.01% |
| 21 | `mulps` | 10 | 0.01% |
| 22 | `divps` | 10 | 0.01% |
| 23 | `subl (lock operand)` | 6 | 0.00% |
| 24 | `addl (lock operand)` | 6 | 0.00% |
| 25 | `cvtdq2pd` | 5 | 0.00% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 851 | 0.56% | `cvtsi2ssl`, `addss`, `cvttss2si`, `mulss`, `pmuludq`, `mulsd` |
...
- **Covered by the model: 652,677 (90.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 69,306 (9.6%).
...
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 98.6% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 52,327 | 7.24% | `psubusw`, `pmaddwd`, `psadbw`, `pmullw`, `pmuludq`, `packssdw` |
...
- **Covered by the model: 330,965 (87.7%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 34,012 (9.0%).
...
| `CC` | both routes say compiler-emitted | 269,143 | 71.33% | 98.2% |
...
| SSE-legacy (xmm) | 10,014 | 2.65% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `pmuludq` |
...
- **4,784,269 instructions**, covered by the model **4,385,798 (91.7%)**.
...
- **Not covered: 398,471 instructions (8.3% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 132,309 | 2.77% | 33.2% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 27.2% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 15.6% |
| MMX (mm) | 42,881 | 0.90% | 10.8% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 8.5% |
| CET-IBT | 16,488 | 0.34% | 4.1% |
