...
<!-- census-model: mnemonics=111 sha=dace355a7b711194 rules=de60cb1da35d8a41 -->
...
The model covers **111 mnemonics**. Every number below is a STATIC count of instruction occurrences in a disassembly, not a dynamic profile: it measures what a binary CONTAINS, which is the right question for deciding what to model next and the wrong one for deciding what is hot.
...
- **Covered by the model: 5,370,988 (99.8%).**
...
- Not mapped at all, and therefore counted as NOT covered: 8,853 (0.2%).
...
| 17 | `cvtsi2ss` | 41 | 0.00% |
| 18 | `incsspq` | 28 | 0.00% |
| 19 | `xaddl (lock operand)` | 28 | 0.00% |
| 20 | `orps` | 27 | 0.00% |
| 21 | `cmpxchgq (lock operand)` | 24 | 0.00% |
| 22 | `movlpd` | 23 | 0.00% |
| 23 | `addl (lock operand)` | 22 | 0.00% |
| 24 | `cvtss2sd` | 19 | 0.00% |
| 25 | `cvttsd2si` | 18 | 0.00% |
...
|---|---|---|---|
| SSE-legacy (xmm) | 7,251 | 0.13% | `movhps`, `xorps`, `pshufd`, `shufpd`, `psrldq`, `movhlps` |
...
- **Covered by the model: 863,196 (98.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 16,355 (1.9%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 344 | 0.04% | 94.5% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 1,155 | 0.13% | 68.6% |
| `CC` | both routes say compiler-emitted | 863,382 | 98.16% | 98.1% |
...
| SSE-legacy (xmm) | 2,338 | 0.27% | `comiss`, `cvtsi2ss`, `cvttss2si`, `mulss`, `addss`, `cvtsi2sd` |
...
- **Covered by the model: 533,397 (88.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 69,485 (11.5%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 20,563 | 3.41% | 56.7% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 13,783 | 2.28% | 62.6% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 55,619 | 9.22% | 63.2% |
| `CC` | both routes say compiler-emitted | 504,808 | 83.64% | 92.9% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 26,917 | 4.46% | `movapd`, `mulsd`, `addsd`, `subsd`, `pcmpeqb`, `pmovmskb` |
...
- **Covered by the model: 185,840 (61.2%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 117,930 (38.8%).
...
| 15 | `psrad` | 1,630 | 0.54% |
| 16 | `vmovdqu` | 1,611 | 0.53% |
| 17 | `vpunpcklwd` | 1,600 | 0.53% |
| 18 | `vpdpwssd` | 1,589 | 0.52% |
| 19 | `pmaddubsw` | 1,483 | 0.49% |
| 20 | `vpunpckhwd` | 1,415 | 0.47% |
| 21 | `movhps` | 1,400 | 0.46% |
| 22 | `vmovq` | 1,385 | 0.46% |
| 23 | `vpsubw` | 1,367 | 0.45% |
| 24 | `pshufb` | 1,347 | 0.44% |
| 25 | `vpaddsw` | 1,238 | 0.41% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,083 | 59.60% | 39.6% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 3,619 | 1.19% | 80.9% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 24,700 | 8.13% | 72.3% |
| `CC` | both routes say compiler-emitted | 93,471 | 30.76% | 98.9% |
...
| SSE-legacy (xmm) | 24,194 | 7.96% | `pmaddwd`, `pmulhrsw`, `psrad`, `pmaddubsw`, `movhps`, `pshufb` |
...
- **Covered by the model: 2,938,143 (94.3%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 14,390 (0.46%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 164,453 (5.3%).
...
| 7 | `psrad` | 4,904 | 0.16% |
| 8 | `addss` | 4,384 | 0.14% |
| 9 | `vshufps` | 4,340 | 0.14% |
| 10 | `vpaddd` | 4,275 | 0.14% |
| 11 | `mulsd` | 4,181 | 0.13% |
| 12 | `vpmaddwd` | 4,034 | 0.13% |
| 13 | `vpaddw` | 3,966 | 0.13% |
| 14 | `addsd` | 3,468 | 0.11% |
| 15 | `vaddps` | 3,418 | 0.11% |
| 16 | `vpsrad` | 2,949 | 0.09% |
| 17 | `vsubps` | 2,791 | 0.09% |
| 18 | `vmulps` | 2,620 | 0.08% |
| 19 | `cvtss2sd` | 2,607 | 0.08% |
| 20 | `packssdw` | 2,595 | 0.08% |
| 21 | `vpsubw` | 2,456 | 0.08% |
| 22 | `paddw (vector operand)` | 2,303 | 0.07% |
| 23 | `vmovdqu` | 2,176 | 0.07% |
| 24 | `subss` | 2,132 | 0.07% |
| 25 | `psraw` | 2,097 | 0.07% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 227,546 | 7.30% | 42.0% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 133 | 0.00% | 72.9% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 127,628 | 4.09% | 95.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 68,231 | 2.19% | `pmaddwd`, `mulss`, `psrad`, `addss`, `mulsd`, `addsd` |
...
- **Covered by the model: 149,237 (98.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 2,439 (1.6%).
...
| `CC` | both routes say compiler-emitted | 150,388 | 99.14% | 98.4% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 1,618 | 1.07% | `pshufd`, `cvtsi2ss`, `addss`, `cvttss2si`, `mulss`, `pmuludq` |
...
- **Covered by the model: 85,633 (76.4%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 4,263 (3.80%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 22,208 (19.8%).
...
|---|---|---|---|
| 1 | `psrlw` | 3,708 | 3.31% |
| 2 | `packuswb` | 3,496 | 3.12% |
| 3 | `psllw` | 3,002 | 2.68% |
| 4 | `pshufd` | 2,921 | 2.61% |
| 5 | `paddsw` | 2,016 | 1.80% |
| 6 | `movntdqa` | 1,984 | 1.77% |
| 7 | `movq (vector operand)` | 1,867 | 1.67% |
| 8 | `pmulhw` | 1,728 | 1.54% |
| 9 | `punpcklbw (vector operand)` | 897 | 0.80% |
| 10 | `movntdq` | 690 | 0.62% |
| 11 | `psubsw` | 576 | 0.51% |
| 12 | `movhpd` | 512 | 0.46% |
| 13 | `movd (vector operand)` | 436 | 0.39% |
| 14 | `punpckhbw (vector operand)` | 327 | 0.29% |
| 15 | `psubusb` | 288 | 0.26% |
| 16 | `pshufb` | 256 | 0.23% |
| 17 | `movlpd` | 256 | 0.23% |
| 18 | `prefetchnta` | 237 | 0.21% |
| 19 | `endbr64` | 235 | 0.21% |
| 20 | `pxor (vector operand)` | 224 | 0.20% |
| 21 | `pand (vector operand)` | 192 | 0.17% |
| 22 | `movhlps` | 132 | 0.12% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `CA` | packed-SIMD body, but no ISA suffix in the name | 59,515 | 53.09% | 65.7% |
| `CC` | both routes say compiler-emitted | 52,317 | 46.67% | 88.4% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 18,990 | 16.94% | `psrlw`, `pshufd`, `packuswb`, `psllw`, `movntdqa`, `paddsw` |
...
- **Covered by the model: 625,775 (86.6%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 316 (0.04%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 96,240 (13.3%).
...
| 5 | `pshufd` | 3,866 | 0.54% |
| 6 | `pmullw` | 2,727 | 0.38% |
| 7 | `endbr64` | 2,345 | 0.32% |
| 8 | `pmuludq` | 2,332 | 0.32% |
| 9 | `psrldq` | 2,291 | 0.32% |
| 10 | `packssdw` | 1,861 | 0.26% |
| 11 | `vmovdqa` | 1,822 | 0.25% |
| 12 | `vmovdqu` | 1,729 | 0.24% |
| 13 | `pcmpgtd` | 1,675 | 0.23% |
| 14 | `pcmpeqw` | 1,627 | 0.23% |
| 15 | `psraw` | 1,577 | 0.22% |
| 16 | `pmulhuw` | 1,391 | 0.19% |
| 17 | `paddsw` | 1,351 | 0.19% |
| 18 | `vpaddd` | 1,274 | 0.18% |
| 19 | `vpaddw` | 1,243 | 0.17% |
| 20 | `pandn` | 1,182 | 0.16% |
| 21 | `pmuldq` | 1,058 | 0.15% |
| 22 | `vpsubw` | 947 | 0.13% |
| 23 | `shufps` | 854 | 0.12% |
| 24 | `pshuflw` | 752 | 0.10% |
| 25 | `packuswb` | 752 | 0.10% |
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 181,880 | 25.18% | 66.2% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 81,330 | 11.26% | 79.4% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 31,862 | 4.41% | 87.8% |
| `CC` | both routes say compiler-emitted | 412,101 | 57.05% | 97.3% |
| `U` | no function symbol covers this address | 15,158 | 2.10% | 79.0% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 79,179 | 10.96% | `psubusw`, `pmaddwd`, `psrad`, `psadbw`, `pshufd`, `pmullw` |
...
- **Covered by the model: 324,905 (86.1%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
- Mapped to a roster name the model HAS, but in a form its state cannot represent — an MMX, `%ymm`, `%zmm`, `%k` or x87 register, a VEX encoding, or a `lock` prefix (whose legality is per-form and not published here): 11,633 (3.08%) — counted as NOT covered.
- Not mapped at all, and therefore counted as NOT covered: 40,792 (10.8%).
...
| cell | what it means | instructions | share | covered by the model |
|---|---|---|---|---|
| `AA` | both routes say HAND-WRITTEN | 72,569 | 19.23% | 41.9% |
| `CA` | packed-SIMD body, but no ISA suffix in the name | 572 | 0.15% | 72.7% |
| `AC` | ISA-suffixed name, but the body is not packed SIMD | 33,250 | 8.81% | 87.5% |
...
| bucket | occurrences | share of column | the mnemonics in it |
|---|---|---|---|
| SSE-legacy (xmm) | 15,896 | 4.21% | `psadbw`, `pmaxsw`, `pabsw`, `mulsd`, `pmaddwd`, `psrld` |
...
- **4,784,269 instructions**, covered by the model **4,309,533 (90.1%)**.
...
- **Not covered: 474,736 instructions (9.9% of the class)** — this is the P2 demand, and the table below is it, per extension.

| bucket | occurrences | share of the class | share of the UNCOVERED work |
|---|---|---|---|
| SSE-legacy (xmm) | 208,108 | 4.35% | 43.8% |
| AVX2/AVX (ymm) | 108,554 | 2.27% | 22.9% |
| VEX-128 (v… xmm) | 62,252 | 1.30% | 13.1% |
| MMX (mm) | 42,881 | 0.90% | 9.0% |
| AVX-512 (zmm/k) | 33,884 | 0.71% | 7.1% |
| CET-IBT | 16,488 | 0.34% | 3.5% |
...
- **Covered by the model: 2,767,106 (99.5%).**
- Mapped to a roster name the model does not have: 0 (0.0%).
...
- Not mapped at all, and therefore counted as NOT covered: 5,769 (0.2%).
...
| SSE-legacy (xmm) | 18 | 0.00% | `pshufd`, `pshufb`, `psrld`, `pslld` |
