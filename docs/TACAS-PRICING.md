...
| G6 | **Related work.** ⚠️ **NOT "not started" — a 10-element PRIOR-ART TABLE already exists** (this campaign's 2026-09-02 provenance verdict, a private-lane document), with named public prior art per design element and an evidence class on each: Myreen FMCAD 2012 · Dasgupta PLDI 2019 · Heule PLDI 2016 · Armstrong POPL 2019 · Verbeek/Roessle/Bockenek CPP 2019 + PLDI 2022 · LNSym · seL4/AutoCorres. | required | ✅ **RE-DRIVEN AND TRANSPLANTED 2026-09-11 → `docs/TACAS-G6-RELATED-WORK.md`** (D201). **Discovery and verification are now SPENT; what remains is bibliography and prose.** ~~Still ~2 days~~ ⚖️ **RE-PRICED 2026-09-12 → ~1:** the two paragraphs resting on measured cells are PROSE in `paper/x86lean-semantics.tex` §7, and every DOI record was fetched per entry (which found **3 of 8 rows wrong**, D214). **Left:** the Myreen record (needs a browser), a read of Roessle/Verbeek beyond the over-approximation claim, and the frame-discipline sentences. ✅ **2026-09-12 (D222): the Myreen record is VERIFIED without a browser (the author's own page) and the frame sentence is PROSE in §7 — citing TACAS 2007 and FMCAD 2008 for the frame rule, because reading FMCAD 2012 showed it is not there.** Still left, and not re-priced here: Roessle/Verbeek beyond abstracts · seL4/AutoCorres · Armstrong POPL 2019 · per-form fidelity precedents (the §7 placeholder's remaining items). ⚖️ **2026-09-14, re-read against the paper rather than this cell:** **seL4/AutoCorres — DROPPED BY THE AUTHOR, not deferred.** The paper's frame precedent for MACHINE CODE is Myreen and Gordon (TACAS 2007, FMCAD 2008, FMCAD 2012), read at the author's PDFs (D222) and cited in §7. seL4 and AutoCorres verify C, and a citation for "practice" would claim an influence that no sentence in the paper rests on. Neither is in `paper/x86lean-semantics.bib`; both records stay in `TACAS-G6-RELATED-WORK.md` §5 in case a reviewer asks. **Armstrong POPL 2019 — DONE** (read at the PDF, D223; cited in §7). **Still left:** Roessle/Verbeek beyond their abstracts (the over-approximation sentence rests on the abstracts, and says so in its `\src`). ⇒ **G6 re-priced ~1 → ~0.5.** ✅ **2026-09-14, later: DONE (D246).** Both papers read at the authors' PDFs, titles and authors matched to their DOI records. **The read changed three sentences in §7, not one:** the fidelity sentence dropped `verbeek-lifting` (its over-approximation is of a binary's EXECUTION PATHS, and it ASSUMES sound instruction semantics); Roessle's 1,625 are instruction VARIANTS, generalized from 692 learned instructions; and CPP 2019 §6 (hardware-computed test lemmas proven in Isabelle/HOL) is now cited as validation precedent. Page count unchanged (15, References on 14). ⇒ **G6 re-priced ~0.5 → 0.** |
...
```
  BEFORE OCT 15:  G6 0 · G7 0 · data statement: two EXTERNAL items (the submission commit; the archive DOI)  ⇒  ~0 working days
      (G7 CLOSED 09-15, D250: the last full read was a second non-author read, 14 findings, all confirmed and applied;
       the data statement's own text was brought up to date in the same landing)
      ...
      (G6 re-priced 0.5 -> 0 later on 09-14: Roessle/Verbeek read at the PDFs, three §7 sentences changed, D246)
