...
acc,imm · rh"
  ...
  -- than left for a reader to assume they were separately tested.
  --
  -- ⭐⭐ P2 ITEM 2: THE MEMORY SHAPE IS NO LONGER REFUSED.  This row read
  -- `(m: refused, implicit LOCK)` for eleven batches — D25's decline, because
  -- the implicit LOCK is an atomicity claim the model had no vocabulary for.
  -- `Ea.lock` is that vocabulary; the shape is modelled now, and the atomicity
  -- is RECORDED rather than verified (`TRUSTBASE.md`).  ⚠️ The note stays in the
  ...
  , { mnemonic := "cvtpd2ps",
      shapes := "x,x · x,m", note := "two binary64 lanes narrowed under MXCSR.RC into the low two binary32 lanes, bits 127:64 zeroed, flags ORed; m128 16-byte aligned (#GP); D294, D295",
      tier := .exact, decode := .xed, undefined := [], sdm := "Vol. 2A CVTPD2PS" }
