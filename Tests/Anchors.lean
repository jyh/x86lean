...
/-! ## ⭐⭐⭐ THE STICKY EXCEPTION FLAGS WHERE NO VECTOR REACHES — SUB-GROUP B0 (D266)

`step` on the landed FP forms, with MXCSR loaded from the row, checked for the result, the sticky
flags it ORs in, and that it did not refuse.  The rows are `hwprobe/`'s, and ONLY THE 40 OF ITS 89
THE DIFFERENTIAL CANNOT CARRY (D267 §2): each is one x86isa disagrees with or cannot run, or one
whose operand class no vector of its form reaches over the 88 pre-states.  That is COMIS at a QNaN,
every SNaN at a comparison or min/max, a denormal beside a QNaN, a preset OE at `cvtss2sd`, integer
0 at round-down, and INT_MIN exactly.  Each row names its reason.

⛔ **THE EXPECTATIONS ARE NOT THE MODEL'S, AND THEY ARE NOT TYPED.** The block below is generated
by `hwprobe/mk_anchors.py` from `hwprobe/mk_rows.py`, which derives each value and flag from the
SDM's exception lists and NaN rules, and `mk_anchors.py --check` holds this file to its output.
Each row has a second origin: an AMD EPYC 7763 ran all of them and agreed (D266 §5).
Of all 89, **x86isa runs 88 and disagrees on 19, in exactly three mechanisms** (COMIS at a QNaN 6 ·
a preset OE read as overflow 12 · integer 0 → −0 at round-down 1), and every one of the 19 is pinned
here; the `cvtss2sd` row at −0 is not run there (D258).  Here the model answers to the rules and the
processor, not to the oracle.

⛔ **"COMIS AS UCOMIS" IS REFUTED HERE AND NOWHERE ELSE.** x86isa runs COMIS as UCOMIS, so no
vector can tell the two models apart (D266 §6.7); `b0_comis_pins` does, at every QNaN row.

⛔ **PLANTED WRONG ONCE EACH:** see D267.

⭐⭐⭐ **SUB-GROUP B1 (D268) ADDS THE MULTIPLY: 44 OF hwprobe's 85 `mulsd`/`mulss` ROWS**, by the same
rule. 8 are ∞ × 0, where x86isa's indefinite is POSITIVE and the SDM's (and both processors') is
negative. The other 36 are classes the 11 multiply vectors reach in no state:
- an exact product with no denormal operand;
- a tie at round-to-nearest, and a tie whose rounding carries;
- overflow at round-up;
- every SNaN;
- TININESS BEFORE ROUNDING THAT IS NOT TININESS AFTER (`tinyup`), the rule D266 §3 measured x86isa using;
- an exact tiny result at three of the modes;
- a preset OE (`mulsd_sticky_or`: every flag preset, nothing new raised).
The class is the reference path, flags, RC, sign, direction and tininess split (D268 §2). Planted
wrong once each: see D268.

⚠️ A different instrument from the vector table, never pooled with it.  The pre-state is the
probe's: `a` in %rdi and in %xmm0's low quadword, `b` in %xmm1's, both upper quadwords zero. -/

/-- The probe's pre-state (`hwprobe/sse_ops.S`), with MXCSR from the row. -/
private def b0Pre (mx : BitVec 32) (a b : BitVec 64) : Cpu :=
  { ((mk { rdi := a }).setXmm .x0 (a.setWidth 128)).setXmm .x1 (b.setWidth 128) with mxcsr := mx }

/-- The RFLAGS image of the flags this model keeps, at `flagFields`' positions. -/
private def b0Rflags (f : Flags) : BitVec 64 :=
  flagFields.foldl (fun acc r => acc ||| (if r.get f then (1 : BitVec 64) <<< r.bit else 0)) 0

/-- A row agrees when the step did not refuse, its result is the row's, and MXCSR is the row's
value with the row's flags ORed in (the probe's `want_mx`). -/
private def b0Agrees (t : Cpu) (mx fl : BitVec 32) (got want : BitVec 64) : Bool :=
  t.ms.isNone && got == want && t.mxcsr == (mx ||| fl)

/-- B5's probe pre-state: all 128 bits of both sources, as `LOAD4` leaves them (`hwprobe/sse_ops.S`, D281 §1). -/
private def b5Pre (mx : BitVec 32) (a b : BitVec 128) : Cpu :=
  { ((mk {}).setXmm .x0 a).setXmm .x1 b with mxcsr := mx }

/-- `b0Agrees` at 128 bits: every bit of the destination is the row's. -/
private def b5Agrees (t : Cpu) (mx fl : BitVec 32) (got want : BitVec 128) : Bool :=
  t.ms.isNone && got == want && t.mxcsr == (mx ||| fl)

-- ⟦B0–B4 PINS BEGIN — GENERATED by hwprobe/mk_anchors.py; change mk_rows.py and regenerate⟧

theorem b0_comis_pins :
  ([  -- 19 of hwprobe's 36 rows
    (true, .q, 0x1f80, 0x7ff8000000000000, 0x3ff0000000000000, 0x045, 0x01),  -- comisd_qnan_one · x86isa differs
    (true, .q, 0x1f80, 0x3ff0000000000000, 0x7ff8000000000000, 0x045, 0x01),  -- comisd_one_qnan · x86isa differs
    (true, .q, 0x1f80, 0x7ff0000000000789, 0x3ff0000000000000, 0x045, 0x01),  -- comisd_snan_one · no vector reaches
    (true, .q, 0x1f80, 0x7ff8000000000000, 0x000fffffffffffff, 0x045, 0x01),  -- comisd_qnan_den · x86isa differs
    (true, .q, 0x1f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x045, 0x01),  -- comisd_snan_den · no vector reaches
    (false, .q, 0x1f80, 0x7ff0000000000789, 0x3ff0000000000000, 0x045, 0x01),  -- ucomisd_snan_one · no vector reaches
    (false, .q, 0x1f80, 0x7ff8000000000000, 0x000fffffffffffff, 0x045, 0x00),  -- ucomisd_qnan_den · no vector reaches
    (false, .q, 0x1f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x045, 0x01),  -- ucomisd_snan_den · no vector reaches
    (false, .q, 0x1f80, 0x3ff0000000000000, 0x3ff0000000000000, 0x040, 0x00),  -- ucomisd_one_one · no vector reaches
    (true, .d, 0x1f80, 0x000000007fc00000, 0x000000003f800000, 0x045, 0x01),  -- comiss_qnan_one · x86isa differs
    (true, .d, 0x1f80, 0x000000003f800000, 0x000000007fc00000, 0x045, 0x01),  -- comiss_one_qnan · x86isa differs
    (true, .d, 0x1f80, 0x000000007f800123, 0x000000003f800000, 0x045, 0x01),  -- comiss_snan_one · no vector reaches
    (true, .d, 0x1f80, 0x000000007fc00000, 0x00000000007fffff, 0x045, 0x01),  -- comiss_qnan_den · x86isa differs
    (true, .d, 0x1f80, 0x000000007f800123, 0x00000000007fffff, 0x045, 0x01),  -- comiss_snan_den · no vector reaches
    (true, .d, 0x1f80, 0x000000003f800000, 0x000000003f800000, 0x040, 0x00),  -- comiss_one_one · no vector reaches
    (false, .d, 0x1f80, 0x000000007f800123, 0x000000003f800000, 0x045, 0x01),  -- ucomiss_snan_one · no vector reaches
    (false, .d, 0x1f80, 0x000000007fc00000, 0x00000000007fffff, 0x045, 0x00),  -- ucomiss_qnan_den · no vector reaches
    (false, .d, 0x1f80, 0x000000007f800123, 0x00000000007fffff, 0x045, 0x01),  -- ucomiss_snan_den · no vector reaches
    (false, .d, 0x1f80, 0x000000003f800000, 0x000000003f800000, 0x040, 0x00)  -- ucomiss_one_one · no vector reaches
  ] : List (Bool × Size × BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (o, sz, mx, a, b, e, fl) =>
      let t := step ⟨.vcomis o sz .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl (b0Rflags t.flags &&& 0x8d5) e) = true := by decide

theorem b0_minmax_pins :
  ([  -- 2 of hwprobe's 12 rows
    (.q, 0x1f80, 0x7ff0000000000789, 0x3ff0000000000000, 0x3ff0000000000000, 0xffffffffffffffff, 0x01),  -- minsd_snan_one · no vector reaches
    (.d, 0x1f80, 0x000000007f800123, 0x000000003f800000, 0x000000003f800000, 0x00000000ffffffff, 0x01)  -- minss_snan_one · no vector reaches
  ] : List (Size × BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (sz, mx, a, b, r, m, fl) =>
      let t := step ⟨.vminmax false sz .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64 &&& m) r) = true := by decide

theorem b0_cvtss2sd_pins :
  ([  -- 13 of hwprobe's 17 rows
    (0x1f80, 0x0000000080000000, 0x8000000000000000, 0x00),  -- cvtss2sd_negzero · x86isa cannot run it
    (0x1f88, 0x000000003f800000, 0x3ff0000000000000, 0x00),  -- cvtss2sd_one_sticky1f88 · x86isa differs
    (0x1fbf, 0x000000003f800000, 0x3ff0000000000000, 0x00),  -- cvtss2sd_one_sticky1fbf · x86isa differs
    (0x5f88, 0x000000003f800000, 0x3ff0000000000000, 0x00),  -- cvtss2sd_one_sticky5f88 · x86isa differs
    (0x7f88, 0x000000003f800000, 0x3ff0000000000000, 0x00),  -- cvtss2sd_one_sticky7f88 · x86isa differs
    (0x1f88, 0x0000000011111111, 0x3a22222220000000, 0x00),  -- cvtss2sd_small_sticky1f88 · x86isa differs
    (0x1fbf, 0x0000000011111111, 0x3a22222220000000, 0x00),  -- cvtss2sd_small_sticky1fbf · x86isa differs
    (0x5f88, 0x0000000011111111, 0x3a22222220000000, 0x00),  -- cvtss2sd_small_sticky5f88 · x86isa differs
    (0x7f88, 0x0000000011111111, 0x3a22222220000000, 0x00),  -- cvtss2sd_small_sticky7f88 · x86isa differs
    (0x1f88, 0x0000000091111111, 0xba22222220000000, 0x00),  -- cvtss2sd_negsmall_sticky1f88 · x86isa differs
    (0x1fbf, 0x0000000091111111, 0xba22222220000000, 0x00),  -- cvtss2sd_negsmall_sticky1fbf · x86isa differs
    (0x5f88, 0x0000000091111111, 0xba22222220000000, 0x00),  -- cvtss2sd_negsmall_sticky5f88 · x86isa differs
    (0x7f88, 0x0000000091111111, 0xba22222220000000, 0x00)  -- cvtss2sd_negsmall_sticky7f88 · x86isa differs
...
theorem b0_cvtsi2sd_pins :
  ([  -- 3 of hwprobe's 16 rows
    (0x3f80, 0x0000000000000000, 0x0000000000000000, 0x00),  -- cvtsi2sd_zero/down · x86isa differs
    (0x7f80, 0x0000000000000001, 0x3ff0000000000000, 0x00),  -- cvtsi2sd_one/zero · no vector reaches
...
theorem b0_cvttsd2si_pins :
  ([  -- 3 of hwprobe's 8 rows
    (0x1f80, 0x7ff0000000000789, 0x0000000080000000, 0x01),  -- cvttsd2si_snan · no vector reaches
    (0x1f80, 0xc1e0000000000000, 0x0000000080000000, 0x00),  -- cvttsd2si_negtwo31 · no vector reaches
    (0x1f80, 0x3ff0000000000000, 0x0000000000000001, 0x00)  -- cvttsd2si_one · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, r, fl) =>
      let t := step ⟨.vcvtt2si true false .rax .x0, 4⟩ (b0Pre mx a 0)
      b0Agrees t mx fl (t.regs.rax &&& 0xffffffff) r) = true := by decide

theorem b1_mulsd_pins :
  ([  -- 31 of hwprobe's 65 rows
    (0x1f80, 0x3ff8000000000000, 0x4000000000000000, 0x4008000000000000, 0x00),  -- mulsd_exact/nearest · no vector reaches
    (0x3f80, 0x3ff8000000000000, 0x4000000000000000, 0x4008000000000000, 0x00),  -- mulsd_exact/down · no vector reaches
    (0x5f80, 0x3ff8000000000000, 0x4000000000000000, 0x4008000000000000, 0x00),  -- mulsd_exact/up · no vector reaches
    (0x7f80, 0x3ff8000000000000, 0x4000000000000000, 0x4008000000000000, 0x00),  -- mulsd_exact/zero · no vector reaches
    (0x1f80, 0x3fd5555555555555, 0x4008000000000000, 0x3ff0000000000000, 0x20),  -- mulsd_tiecarry/nearest · no vector reaches
    (0x5f80, 0x3fd5555555555555, 0x4008000000000000, 0x3ff0000000000000, 0x20),  -- mulsd_tiecarry/up · no vector reaches
    (0x1f80, 0x3ff0000000000001, 0x3ff8000000000000, 0x3ff8000000000002, 0x20),  -- mulsd_tieodd/nearest · no vector reaches
    (0x1f80, 0xbff0000000000001, 0x3ff8000000000000, 0xbff8000000000002, 0x20),  -- mulsd_negtie/nearest · no vector reaches
    (0x5f80, 0x7fefffffffffffff, 0x4000000000000000, 0x7ff0000000000000, 0x28),  -- mulsd_overflow/up · no vector reaches
    (0x1f80, 0x8000000000000001, 0x3fe0000000000000, 0x8000000000000000, 0x32),  -- mulsd_negsubtie/nearest · no vector reaches
    (0x1f80, 0x7ff0000000000000, 0x8000000000000000, 0xfff8000000000000, 0x01),  -- mulsd_invalid/nearest · x86isa differs
    (0x3f80, 0x7ff0000000000000, 0x8000000000000000, 0xfff8000000000000, 0x01),  -- mulsd_invalid/down · x86isa differs
    (0x5f80, 0x7ff0000000000000, 0x8000000000000000, 0xfff8000000000000, 0x01),  -- mulsd_invalid/up · x86isa differs
    (0x7f80, 0x7ff0000000000000, 0x8000000000000000, 0xfff8000000000000, 0x01),  -- mulsd_invalid/zero · x86isa differs
    (0x1f80, 0x7ff0000000000789, 0x7ff8000000000abc, 0x7ff8000000000789, 0x01),  -- mulsd_snanqnan/nearest · no vector reaches
    (0x3f80, 0x7ff0000000000789, 0x7ff8000000000abc, 0x7ff8000000000789, 0x01),  -- mulsd_snanqnan/down · no vector reaches
    (0x5f80, 0x7ff0000000000789, 0x7ff8000000000abc, 0x7ff8000000000789, 0x01),  -- mulsd_snanqnan/up · no vector reaches
    (0x7f80, 0x7ff0000000000789, 0x7ff8000000000abc, 0x7ff8000000000789, 0x01),  -- mulsd_snanqnan/zero · no vector reaches
    (0x1f80, 0x3ff0000000000001, 0x000fffffffffffff, 0x0010000000000000, 0x22),  -- mulsd_tinyup/nearest · no vector reaches
    (0x5f80, 0x3ff0000000000001, 0x000fffffffffffff, 0x0010000000000000, 0x22),  -- mulsd_tinyup/up · no vector reaches
    (0x1f80, 0x000fffffffffffff, 0x3ff0000000000001, 0x0010000000000000, 0x22),  -- mulsd_tinyup_swap/nearest · no vector reaches
    (0x5f80, 0x000fffffffffffff, 0x3ff0000000000001, 0x0010000000000000, 0x22),  -- mulsd_tinyup_swap/up · no vector reaches
    (0x1f80, 0x0008000000000000, 0x3ff0000000000000, 0x0008000000000000, 0x02),  -- mulsd_exacttiny/nearest · no vector reaches
    (0x3f80, 0x0008000000000000, 0x3ff0000000000000, 0x0008000000000000, 0x02),  -- mulsd_exacttiny/down · no vector reaches
    (0x5f80, 0x0008000000000000, 0x3ff0000000000000, 0x0008000000000000, 0x02),  -- mulsd_exacttiny/up · no vector reaches
    (0x1f80, 0x0000000000000003, 0x3fe0000000000000, 0x0000000000000002, 0x32),  -- mulsd_deeptiny/nearest · no vector reaches
    (0x1f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x7ff8000000000789, 0x01),  -- mulsd_snan_den/nearest · no vector reaches
    (0x3f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x7ff8000000000789, 0x01),  -- mulsd_snan_den/down · no vector reaches
    (0x5f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x7ff8000000000789, 0x01),  -- mulsd_snan_den/up · no vector reaches
    (0x7f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x7ff8000000000789, 0x01),  -- mulsd_snan_den/zero · no vector reaches
...
theorem b1_mulss_pins :
  ([  -- 13 of hwprobe's 20 rows
    (0x1f80, 0x000000003fc00000, 0x0000000040000000, 0x0000000040400000, 0x00),  -- mulss_exact/nearest · no vector reaches
    (0x3f80, 0x000000003fc00000, 0x0000000040000000, 0x0000000040400000, 0x00),  -- mulss_exact/down · no vector reaches
    (0x5f80, 0x000000003fc00000, 0x0000000040000000, 0x0000000040400000, 0x00),  -- mulss_exact/up · no vector reaches
    (0x7f80, 0x000000003fc00000, 0x0000000040000000, 0x0000000040400000, 0x00),  -- mulss_exact/zero · no vector reaches
    (0x1f80, 0x000000007f800000, 0x0000000080000000, 0x00000000ffc00000, 0x01),  -- mulss_invalid/nearest · x86isa differs
    (0x3f80, 0x000000007f800000, 0x0000000080000000, 0x00000000ffc00000, 0x01),  -- mulss_invalid/down · x86isa differs
    (0x5f80, 0x000000007f800000, 0x0000000080000000, 0x00000000ffc00000, 0x01),  -- mulss_invalid/up · x86isa differs
    (0x7f80, 0x000000007f800000, 0x0000000080000000, 0x00000000ffc00000, 0x01),  -- mulss_invalid/zero · x86isa differs
    (0x1f80, 0x000000003f800001, 0x00000000007fffff, 0x0000000000800000, 0x22),  -- mulss_tinyup/nearest · no vector reaches
    (0x5f80, 0x000000003f800001, 0x00000000007fffff, 0x0000000000800000, 0x22),  -- mulss_tinyup/up · no vector reaches
    (0x3f80, 0x0000000000400000, 0x000000003f800000, 0x0000000000400000, 0x02),  -- mulss_exacttiny/down · no vector reaches
    (0x5f80, 0x0000000000400000, 0x000000003f800000, 0x0000000000400000, 0x02),  -- mulss_exacttiny/up · no vector reaches
...
theorem b2_addsd_pins :
  ([  -- 15 of hwprobe's 39 rows
    (0x1f80, 0x3ff0000000000000, 0x3ca0000000000000, 0x3ff0000000000000, 0x20),  -- addsd_tie/nearest · no vector reaches
    (0x1f80, 0x7fefffffffffffff, 0x7fefffffffffffff, 0x7ff0000000000000, 0x28),  -- addsd_overflow/nearest · no vector reaches
    (0x3f80, 0x7fefffffffffffff, 0x7fefffffffffffff, 0x7fefffffffffffff, 0x28),  -- addsd_overflow/down · no vector reaches
    (0x5f80, 0x7fefffffffffffff, 0x7fefffffffffffff, 0x7ff0000000000000, 0x28),  -- addsd_overflow/up · no vector reaches
    (0x7f80, 0x7fefffffffffffff, 0x7fefffffffffffff, 0x7fefffffffffffff, 0x28),  -- addsd_overflow/zero · no vector reaches
    (0x1f80, 0x3ff0000000000000, 0xbff0000000000000, 0x0000000000000000, 0x00),  -- addsd_cancel/nearest · no vector reaches
    (0x3f80, 0x3ff0000000000000, 0xbff0000000000000, 0x8000000000000000, 0x00),  -- addsd_cancel/down · no vector reaches
    (0x5f80, 0x3ff0000000000000, 0xbff0000000000000, 0x0000000000000000, 0x00),  -- addsd_cancel/up · no vector reaches
    (0x7f80, 0x3ff0000000000000, 0xbff0000000000000, 0x0000000000000000, 0x00),  -- addsd_cancel/zero · no vector reaches
    (0x3f80, 0x0000000000000000, 0x8000000000000000, 0x8000000000000000, 0x00),  -- addsd_zeros/down · no vector reaches
    (0x1f80, 0x8000000000000000, 0x8000000000000000, 0x8000000000000000, 0x00),  -- addsd_negzeros · no vector reaches
    (0x1f80, 0x7ff0000000000000, 0x7ff0000000000000, 0x7ff0000000000000, 0x00),  -- addsd_infinf · no vector reaches
    (0x1f80, 0x7ff0000000000000, 0x000fffffffffffff, 0x7ff0000000000000, 0x02),  -- addsd_inf_den · no vector reaches
    (0x1f80, 0x7ff0000000000789, 0x3ff0000000000000, 0x7ff8000000000789, 0x01),  -- addsd_snan_one · no vector reaches
    (0x1f80, 0x7ff8000000000000, 0x7ff0000000000789, 0x7ff8000000000000, 0x01)  -- addsd_qnan_snan · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.varith .add .q .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64) r) = true := by decide

theorem b2_subsd_pins :
  ([  -- 3 of hwprobe's 14 rows
    (0x1f80, 0x3ff0000000000000, 0x3c30000000000000, 0x3ff0000000000000, 0x20),  -- subsd_sticky/nearest · no vector reaches
    (0x1f80, 0x3ff0000000000000, 0x3c70000000000000, 0x3ff0000000000000, 0x20),  -- subsd_gap56/nearest · no vector reaches
    (0x1f80, 0x7ff0000000000000, 0x7ff0000000000000, 0xfff8000000000000, 0x01)  -- subsd_infinf · x86isa differs
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.varith .sub .q .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64) r) = true := by decide

theorem b2_divsd_pins :
  ([  -- 11 of hwprobe's 21 rows
    (0x3f80, 0x7fefffffffffffff, 0x3fe0000000000000, 0x7fefffffffffffff, 0x28),  -- divsd_overflow/down · no vector reaches
    (0x5f80, 0x7fefffffffffffff, 0x3fe0000000000000, 0x7ff0000000000000, 0x28),  -- divsd_overflow/up · no vector reaches
    (0x7f80, 0x7fefffffffffffff, 0x3fe0000000000000, 0x7fefffffffffffff, 0x28),  -- divsd_overflow/zero · no vector reaches
    (0x5f80, 0x0010000000000000, 0x4008000000000000, 0x0005555555555556, 0x30),  -- divsd_tiny/up · no vector reaches
    (0x7f80, 0x0010000000000000, 0x4008000000000000, 0x0005555555555555, 0x30),  -- divsd_tiny/zero · no vector reaches
    (0x1f80, 0x4008000000000000, 0x4000000000000000, 0x3ff8000000000000, 0x00),  -- divsd_exact · no vector reaches
    (0x1f80, 0x0000000000000000, 0x0000000000000000, 0xfff8000000000000, 0x01),  -- divsd_zero_zero · x86isa differs
    (0x1f80, 0x7ff0000000000000, 0xfff0000000000000, 0xfff8000000000000, 0x01),  -- divsd_inf_inf · x86isa differs
    (0x1f80, 0xbff0000000000000, 0x0000000000000000, 0xfff0000000000000, 0x04),  -- divsd_negone_zero · no vector reaches
    (0x1f80, 0x7ff0000000000789, 0x000fffffffffffff, 0x7ff8000000000789, 0x01),  -- divsd_snan_den · no vector reaches
    (0x1f80, 0x7ff0000000000000, 0x000fffffffffffff, 0x7ff0000000000000, 0x02)  -- divsd_inf_den · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.varith .div .q .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64) r) = true := by decide

theorem b2_addss_pins :
  ([  -- 6 of hwprobe's 21 rows
    (0x3f80, 0x0000000000000000, 0x0000000080000000, 0x0000000080000000, 0x00),  -- addss_zeros/down · no vector reaches
    (0x1f80, 0x000000007f7fffff, 0x000000007f7fffff, 0x000000007f800000, 0x28),  -- addss_overflow/nearest · no vector reaches
    (0x3f80, 0x000000007f7fffff, 0x000000007f7fffff, 0x000000007f7fffff, 0x28),  -- addss_overflow/down · no vector reaches
    (0x5f80, 0x000000007f7fffff, 0x000000007f7fffff, 0x000000007f800000, 0x28),  -- addss_overflow/up · no vector reaches
    (0x7f80, 0x000000007f7fffff, 0x000000007f7fffff, 0x000000007f7fffff, 0x28),  -- addss_overflow/zero · no vector reaches
    (0x1f80, 0x000000007f800123, 0x000000003f800000, 0x000000007fc00123, 0x01)  -- addss_snan_one · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.varith .add .d .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64 &&& 0xffffffff) r) = true := by decide

theorem b2_subss_pins :
  ([  -- 1 of hwprobe's 5 rows
    (0x1f80, 0x000000007f800000, 0x000000007f800000, 0x00000000ffc00000, 0x01)  -- subss_infinf · x86isa differs
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.varith .sub .d .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64 &&& 0xffffffff) r) = true := by decide

theorem b2_divss_pins :
  ([  -- 3 of hwprobe's 10 rows
    (0x1f80, 0x0000000000800000, 0x0000000040400000, 0x00000000002aaaab, 0x30),  -- divss_tiny/nearest · no vector reaches
    (0x5f80, 0x0000000000800000, 0x0000000040400000, 0x00000000002aaaab, 0x30),  -- divss_tiny/up · no vector reaches
    (0x7f80, 0x0000000000800000, 0x0000000040400000, 0x00000000002aaaaa, 0x30)  -- divss_tiny/zero · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.varith .div .d .x0 .x1, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x0).setWidth 64 &&& 0xffffffff) r) = true := by decide

theorem b4_cvtsi2ss_pins :
  ([  -- 32 of hwprobe's 46 rows
    (0x1f80, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800000, 0x20),  -- cvtsi2ss_tie/nearest · no vector reaches
    (0x3f80, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800000, 0x20),  -- cvtsi2ss_tie/down · no vector reaches
    (0x5f80, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800001, 0x20),  -- cvtsi2ss_tie/up · no vector reaches
    (0x7f80, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800000, 0x20),  -- cvtsi2ss_tie/zero · no vector reaches
    (0x1f80, 0x0000000001000003, 0x5a5ac3c300000000, 0x5a5ac3c34b800002, 0x20),  -- cvtsi2ss_tieodd/nearest · no vector reaches
    (0x3f80, 0x0000000001000003, 0x5a5ac3c300000000, 0x5a5ac3c34b800001, 0x20),  -- cvtsi2ss_tieodd/down · no vector reaches
    (0x5f80, 0x0000000001000003, 0x5a5ac3c300000000, 0x5a5ac3c34b800002, 0x20),  -- cvtsi2ss_tieodd/up · no vector reaches
    (0x7f80, 0x0000000001000003, 0x5a5ac3c300000000, 0x5a5ac3c34b800001, 0x20),  -- cvtsi2ss_tieodd/zero · no vector reaches
    (0x1f80, 0x0000000002000001, 0x5a5ac3c300000000, 0x5a5ac3c34c000000, 0x20),  -- cvtsi2ss_sticky/nearest · no vector reaches
    (0x3f80, 0x0000000002000001, 0x5a5ac3c300000000, 0x5a5ac3c34c000000, 0x20),  -- cvtsi2ss_sticky/down · no vector reaches
    (0x5f80, 0x0000000002000001, 0x5a5ac3c300000000, 0x5a5ac3c34c000001, 0x20),  -- cvtsi2ss_sticky/up · no vector reaches
    (0x7f80, 0x0000000002000001, 0x5a5ac3c300000000, 0x5a5ac3c34c000000, 0x20),  -- cvtsi2ss_sticky/zero · no vector reaches
    (0x1f80, 0x0000000002000003, 0x5a5ac3c300000000, 0x5a5ac3c34c000001, 0x20),  -- cvtsi2ss_above/nearest · no vector reaches
    (0x3f80, 0x0000000002000003, 0x5a5ac3c300000000, 0x5a5ac3c34c000000, 0x20),  -- cvtsi2ss_above/down · no vector reaches
    (0x5f80, 0x0000000002000003, 0x5a5ac3c300000000, 0x5a5ac3c34c000001, 0x20),  -- cvtsi2ss_above/up · no vector reaches
    (0x7f80, 0x0000000002000003, 0x5a5ac3c300000000, 0x5a5ac3c34c000000, 0x20),  -- cvtsi2ss_above/zero · no vector reaches
    (0x1f80, 0x00000000feffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cb800000, 0x20),  -- cvtsi2ss_negtie/nearest · no vector reaches
    (0x3f80, 0x00000000feffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cb800001, 0x20),  -- cvtsi2ss_negtie/down · no vector reaches
    (0x5f80, 0x00000000feffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cb800000, 0x20),  -- cvtsi2ss_negtie/up · no vector reaches
    (0x7f80, 0x00000000feffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cb800000, 0x20),  -- cvtsi2ss_negtie/zero · no vector reaches
    (0x1f80, 0x00000000fdffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cc000000, 0x20),  -- cvtsi2ss_negsticky/nearest · no vector reaches
    (0x3f80, 0x00000000fdffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cc000001, 0x20),  -- cvtsi2ss_negsticky/down · no vector reaches
    (0x5f80, 0x00000000fdffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cc000000, 0x20),  -- cvtsi2ss_negsticky/up · no vector reaches
    (0x7f80, 0x00000000fdffffff, 0x5a5ac3c300000000, 0x5a5ac3c3cc000000, 0x20),  -- cvtsi2ss_negsticky/zero · no vector reaches
    (0x3f80, 0x000000007fffffff, 0x5a5ac3c300000000, 0x5a5ac3c34effffff, 0x20),  -- cvtsi2ss_max/down · no vector reaches
    (0x3f80, 0x0000000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300000000, 0x00),  -- cvtsi2ss_zero/down · x86isa differs
    (0x1f80, 0x0000000000ffffff, 0x5a5ac3c300000000, 0x5a5ac3c34b7fffff, 0x00),  -- cvtsi2ss_exact · no vector reaches
    (0x7f88, 0x0000000000000001, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsi2ss_one_sticky7f88 · no vector reaches
    (0x1f88, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800000, 0x20),  -- cvtsi2ss_tie_sticky1f88 · no vector reaches
    (0x1fbf, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800000, 0x20),  -- cvtsi2ss_tie_sticky1fbf · no vector reaches
    (0x5f88, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800001, 0x20),  -- cvtsi2ss_tie_sticky5f88 · no vector reaches
    (0x7f88, 0x0000000001000001, 0x5a5ac3c300000000, 0x5a5ac3c34b800000, 0x20)  -- cvtsi2ss_tie_sticky7f88 · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vcvtsi2 false false .x1 .rdi, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x1).setWidth 64) r) = true := by decide

theorem b4_cvtsi2ssq_pins :
  ([  -- 25 of hwprobe's 40 rows
    (0x1f80, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_tie/nearest · no vector reaches
    (0x3f80, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_tie/down · no vector reaches
    (0x5f80, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800001, 0x20),  -- cvtsi2ssq_tie/up · no vector reaches
    (0x7f80, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_tie/zero · no vector reaches
    (0x1f80, 0x0000010000000001, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_sticky/nearest · no vector reaches
    (0x3f80, 0x0000010000000001, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_sticky/down · no vector reaches
    (0x5f80, 0x0000010000000001, 0x5a5ac3c300000000, 0x5a5ac3c353800001, 0x20),  -- cvtsi2ssq_sticky/up · no vector reaches
    (0x7f80, 0x0000010000000001, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_sticky/zero · no vector reaches
    (0x1f80, 0xfffffeffffff0000, 0x5a5ac3c300000000, 0x5a5ac3c3d3800000, 0x20),  -- cvtsi2ssq_negtie/nearest · no vector reaches
    (0x3f80, 0xfffffeffffff0000, 0x5a5ac3c300000000, 0x5a5ac3c3d3800001, 0x20),  -- cvtsi2ssq_negtie/down · no vector reaches
    (0x5f80, 0xfffffeffffff0000, 0x5a5ac3c300000000, 0x5a5ac3c3d3800000, 0x20),  -- cvtsi2ssq_negtie/up · no vector reaches
    (0x7f80, 0xfffffeffffff0000, 0x5a5ac3c300000000, 0x5a5ac3c3d3800000, 0x20),  -- cvtsi2ssq_negtie/zero · no vector reaches
    (0x1f80, 0xfffffeffffffffff, 0x5a5ac3c300000000, 0x5a5ac3c3d3800000, 0x20),  -- cvtsi2ssq_negsticky/nearest · no vector reaches
    (0x3f80, 0xfffffeffffffffff, 0x5a5ac3c300000000, 0x5a5ac3c3d3800001, 0x20),  -- cvtsi2ssq_negsticky/down · no vector reaches
    (0x5f80, 0xfffffeffffffffff, 0x5a5ac3c300000000, 0x5a5ac3c3d3800000, 0x20),  -- cvtsi2ssq_negsticky/up · no vector reaches
    (0x7f80, 0xfffffeffffffffff, 0x5a5ac3c300000000, 0x5a5ac3c3d3800000, 0x20),  -- cvtsi2ssq_negsticky/zero · no vector reaches
    (0x3f80, 0x7fffffffffffffff, 0x5a5ac3c300000000, 0x5a5ac3c35effffff, 0x20),  -- cvtsi2ssq_max/down · no vector reaches
    (0x7f80, 0x00000000ffffffff, 0x5a5ac3c300000000, 0x5a5ac3c34f7fffff, 0x20),  -- cvtsi2ssq_width/zero · no vector reaches
    (0x3f80, 0x0000000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300000000, 0x00),  -- cvtsi2ssq_zero/down · x86isa differs
    (0x1f80, 0x0000010000000000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x00),  -- cvtsi2ssq_exact · no vector reaches
    (0x7f88, 0x0000000000000001, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsi2ssq_one_sticky7f88 · no vector reaches
    (0x1f88, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_tie_sticky1f88 · no vector reaches
    (0x1fbf, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20),  -- cvtsi2ssq_tie_sticky1fbf · no vector reaches
    (0x5f88, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800001, 0x20),  -- cvtsi2ssq_tie_sticky5f88 · no vector reaches
    (0x7f88, 0x0000010000010000, 0x5a5ac3c300000000, 0x5a5ac3c353800000, 0x20)  -- cvtsi2ssq_tie_sticky7f88 · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vcvtsi2 false true .x1 .rdi, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x1).setWidth 64) r) = true := by decide

theorem b4_cvtsi2sdq_pins :
  ([  -- 28 of hwprobe's 41 rows
    (0x1f80, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000000, 0x20),  -- cvtsi2sdq_tie/nearest · no vector reaches
    (0x3f80, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000000, 0x20),  -- cvtsi2sdq_tie/down · no vector reaches
    (0x5f80, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000001, 0x20),  -- cvtsi2sdq_tie/up · no vector reaches
    (0x7f80, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000000, 0x20),  -- cvtsi2sdq_tie/zero · no vector reaches
    (0x1f80, 0x0020000000000003, 0x5a5ac3c300000000, 0x4340000000000002, 0x20),  -- cvtsi2sdq_tieodd/nearest · no vector reaches
    (0x3f80, 0x0020000000000003, 0x5a5ac3c300000000, 0x4340000000000001, 0x20),  -- cvtsi2sdq_tieodd/down · no vector reaches
    (0x5f80, 0x0020000000000003, 0x5a5ac3c300000000, 0x4340000000000002, 0x20),  -- cvtsi2sdq_tieodd/up · no vector reaches
    (0x7f80, 0x0020000000000003, 0x5a5ac3c300000000, 0x4340000000000001, 0x20),  -- cvtsi2sdq_tieodd/zero · no vector reaches
    (0x1f80, 0x0040000000000001, 0x5a5ac3c300000000, 0x4350000000000000, 0x20),  -- cvtsi2sdq_sticky/nearest · no vector reaches
    (0x3f80, 0x0040000000000001, 0x5a5ac3c300000000, 0x4350000000000000, 0x20),  -- cvtsi2sdq_sticky/down · no vector reaches
    (0x5f80, 0x0040000000000001, 0x5a5ac3c300000000, 0x4350000000000001, 0x20),  -- cvtsi2sdq_sticky/up · no vector reaches
    (0x7f80, 0x0040000000000001, 0x5a5ac3c300000000, 0x4350000000000000, 0x20),  -- cvtsi2sdq_sticky/zero · no vector reaches
    (0x1f80, 0xffdfffffffffffff, 0x5a5ac3c300000000, 0xc340000000000000, 0x20),  -- cvtsi2sdq_negtie/nearest · no vector reaches
    (0x3f80, 0xffdfffffffffffff, 0x5a5ac3c300000000, 0xc340000000000001, 0x20),  -- cvtsi2sdq_negtie/down · no vector reaches
    (0x5f80, 0xffdfffffffffffff, 0x5a5ac3c300000000, 0xc340000000000000, 0x20),  -- cvtsi2sdq_negtie/up · no vector reaches
    (0x7f80, 0xffdfffffffffffff, 0x5a5ac3c300000000, 0xc340000000000000, 0x20),  -- cvtsi2sdq_negtie/zero · no vector reaches
    (0x1f80, 0xffbfffffffffffff, 0x5a5ac3c300000000, 0xc350000000000000, 0x20),  -- cvtsi2sdq_negsticky/nearest · no vector reaches
    (0x3f80, 0xffbfffffffffffff, 0x5a5ac3c300000000, 0xc350000000000001, 0x20),  -- cvtsi2sdq_negsticky/down · no vector reaches
    (0x5f80, 0xffbfffffffffffff, 0x5a5ac3c300000000, 0xc350000000000000, 0x20),  -- cvtsi2sdq_negsticky/up · no vector reaches
    (0x7f80, 0xffbfffffffffffff, 0x5a5ac3c300000000, 0xc350000000000000, 0x20),  -- cvtsi2sdq_negsticky/zero · no vector reaches
    (0x3f80, 0x7fffffffffffffff, 0x5a5ac3c300000000, 0x43dfffffffffffff, 0x20),  -- cvtsi2sdq_max/down · no vector reaches
    (0x3f80, 0x0000000000000000, 0x5a5ac3c300000000, 0x0000000000000000, 0x00),  -- cvtsi2sdq_zero/down · x86isa differs
    (0x1f80, 0x001fffffffffffff, 0x5a5ac3c300000000, 0x433fffffffffffff, 0x00),  -- cvtsi2sdq_exact · no vector reaches
    (0x7f88, 0x0000000000000001, 0x5a5ac3c300000000, 0x3ff0000000000000, 0x00),  -- cvtsi2sdq_one_sticky7f88 · no vector reaches
    (0x1f88, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000000, 0x20),  -- cvtsi2sdq_tie_sticky1f88 · no vector reaches
    (0x1fbf, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000000, 0x20),  -- cvtsi2sdq_tie_sticky1fbf · no vector reaches
    (0x5f88, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000001, 0x20),  -- cvtsi2sdq_tie_sticky5f88 · no vector reaches
    (0x7f88, 0x0020000000000001, 0x5a5ac3c300000000, 0x4340000000000000, 0x20)  -- cvtsi2sdq_tie_sticky7f88 · no vector reaches
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
...
theorem b3_cvtsd2ss_pins :
  ([  -- 32 of hwprobe's 77 rows
    ...
    (0x3f80, 0x3ff0000010000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x20),  -- cvtsd2ss_tie/down · no vector reaches
    (0x1f80, 0x3ff0000030000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800002, 0x20),  -- cvtsd2ss_tieodd/nearest · no vector reaches
    (0x3f80, 0x3ff0000030000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800001, 0x20),  -- cvtsd2ss_tieodd/down · no vector reaches
    (0x3f80, 0x3ff0000010000001, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x20),  -- cvtsd2ss_sticky/down · no vector reaches
    (0x3f80, 0x3fd5555555555555, 0x5a5ac3c300000000, 0x5a5ac3c33eaaaaaa, 0x20),  -- cvtsd2ss_third/down · no vector reaches
    (0x3f80, 0x47effffff0000000, 0x5a5ac3c300000000, 0x5a5ac3c37f7fffff, 0x20),  -- cvtsd2ss_maxtie/down · no vector reaches
    (0x5f80, 0x3800000000080000, 0x5a5ac3c300000000, 0x5a5ac3c300400001, 0x30),  -- cvtsd2ss_tinyinexact/up · no vector reaches
    (0x1f80, 0x380ffffff8000000, 0x5a5ac3c300000000, 0x5a5ac3c300800000, 0x20),  -- cvtsd2ss_tinyafter/nearest · no vector reaches
    (0x5f80, 0x380ffffff8000000, 0x5a5ac3c300000000, 0x5a5ac3c300800000, 0x20),  -- cvtsd2ss_tinyafter/up · no vector reaches
    (0x1f80, 0x3690000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300000000, 0x30),  -- cvtsd2ss_halfmin/nearest · no vector reaches
    (0x7f80, 0x800fffffffffffff, 0x5a5ac3c300000000, 0x5a5ac3c380000000, 0x32),  -- cvtsd2ss_negden/zero · no vector reaches
    (0x1f80, 0x3ff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsd2ss_one · no vector reaches
    (0x1f80, 0x0000000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300000000, 0x00),  -- cvtsd2ss_zero · x86isa cannot run it
    (0x1f80, 0x8000000000000000, 0x5a5ac3c300000000, 0x5a5ac3c380000000, 0x00),  -- cvtsd2ss_negzero · x86isa cannot run it
    (0x1f80, 0x7ff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c37f800000, 0x00),  -- cvtsd2ss_inf · no vector reaches
    (0x1f80, 0xfff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c3ff800000, 0x00),  -- cvtsd2ss_neginf · no vector reaches
    (0x1f80, 0x7ff8000000000000, 0x5a5ac3c300000000, 0x5a5ac3c37fc00000, 0x00),  -- cvtsd2ss_qnan · no vector reaches
    (0x1f80, 0x7ff0000000000789, 0x5a5ac3c300000000, 0x5a5ac3c37fc00000, 0x01),  -- cvtsd2ss_snan · no vector reaches
    (0x1f80, 0x7ff4000020000000, 0x5a5ac3c300000000, 0x5a5ac3c37fe00001, 0x01),  -- cvtsd2ss_snan_payload · no vector reaches
    (0x1f80, 0x47efffffe0000000, 0x5a5ac3c300000000, 0x5a5ac3c37f7fffff, 0x00),  -- cvtsd2ss_max · no vector reaches
    (0x1f80, 0x3810000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300800000, 0x00),  -- cvtsd2ss_minnorm · no vector reaches
    (0x1f80, 0x36a0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300000001, 0x00),  -- cvtsd2ss_minsub · no vector reaches
    (0x1f80, 0x3800000000000000, 0x5a5ac3c300000000, 0x5a5ac3c300400000, 0x00),  -- cvtsd2ss_exacttiny · no vector reaches
    (0x1f88, 0x3ff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsd2ss_one_sticky1f88 · x86isa differs
    (0x1fbf, 0x3ff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsd2ss_one_sticky1fbf · x86isa differs
    (0x5f88, 0x3ff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsd2ss_one_sticky5f88 · x86isa differs
    (0x7f88, 0x3ff0000000000000, 0x5a5ac3c300000000, 0x5a5ac3c33f800000, 0x00),  -- cvtsd2ss_one_sticky7f88 · x86isa differs
    (0x1f88, 0x3fd5555555555555, 0x5a5ac3c300000000, 0x5a5ac3c33eaaaaab, 0x20),  -- cvtsd2ss_third_sticky1f88 · x86isa differs
    (0x1fbf, 0x3fd5555555555555, 0x5a5ac3c300000000, 0x5a5ac3c33eaaaaab, 0x20),  -- cvtsd2ss_third_sticky1fbf · x86isa differs
    (0x5f88, 0x3fd5555555555555, 0x5a5ac3c300000000, 0x5a5ac3c33eaaaaab, 0x20),  -- cvtsd2ss_third_sticky5f88 · x86isa differs
    (0x7f88, 0x3fd5555555555555, 0x5a5ac3c300000000, 0x5a5ac3c33eaaaaaa, 0x20)  -- cvtsd2ss_third_sticky7f88 · x86isa differs
  ] : List (BitVec 32 × BitVec 64 × BitVec 64 × BitVec 64 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vcvtsd2ss .x1 .x0, 4⟩ (b0Pre mx a b)
      b0Agrees t mx fl ((t.getXmm .x1).setWidth 64) r) = true := by decide

theorem b5_mulps_pins :
  ([  -- 1 of hwprobe's 12 rows
    (0x1f80, 0x7f8000003fc000003fc000003fc00000, 0x00000000400000004000000040000000, 0xffc00000404000004040000040400000, 0x01)  -- mulps_indef@3 · x86isa differs
  ] : List (BitVec 32 × BitVec 128 × BitVec 128 × BitVec 128 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .mul false .x0 .x1, 3⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide

theorem b5_mulpd_pins :
  ([  -- 3 of hwprobe's 10 rows
    (0x1f80, 0x3ff80000000000007ff0000000000789, 0x40000000000000003ff0000000000000, 0x40080000000000007ff8000000000789, 0x01),  -- mulpd_loud@0 · no vector reaches
    (0x1f80, 0x7ff00000000007893ff8000000000000, 0x3ff00000000000004000000000000000, 0x7ff80000000007894008000000000000, 0x01),  -- mulpd_loud@1 · no vector reaches
    (0x1f80, 0x7ff00000000000003ff8000000000000, 0x00000000000000004000000000000000, 0xfff80000000000004008000000000000, 0x01)  -- mulpd_indef@1 · x86isa differs
  ] : List (BitVec 32 × BitVec 128 × BitVec 128 × BitVec 128 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .mul true .x0 .x1, 4⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide

theorem b5_addps_pins :
  ([  -- 5 of hwprobe's 12 rows
    (0x1f80, 0x7fc000007f7fffff3fc000003f800000, 0x007fffff7f7fffff4000000033800000, 0x7fc000007f800000406000003f800000, 0x28),  -- addps_mix/nearest · no vector reaches
    (0x3f80, 0x7fc000007f7fffff3fc000003f800000, 0x007fffff7f7fffff4000000033800000, 0x7fc000007f7fffff406000003f800000, 0x28),  -- addps_mix/down · no vector reaches
    (0x5f80, 0x7fc000007f7fffff3fc000003f800000, 0x007fffff7f7fffff4000000033800000, 0x7fc000007f800000406000003f800001, 0x28),  -- addps_mix/up · no vector reaches
    (0x7f80, 0x7fc000007f7fffff3fc000003f800000, 0x007fffff7f7fffff4000000033800000, 0x7fc000007f7fffff406000003f800000, 0x28),  -- addps_mix/zero · no vector reaches
    (0x1f80, 0x7f8000003fc000003fc000003fc00000, 0xff800000400000004000000040000000, 0xffc00000406000004060000040600000, 0x01)  -- addps_indef@3 · x86isa differs
  ] : List (BitVec 32 × BitVec 128 × BitVec 128 × BitVec 128 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .add false .x0 .x1, 3⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide
...
theorem b5_addpd_pins :
  ([  -- 1 of hwprobe's 10 rows
    ...
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .add true .x0 .x1, 4⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide
...
theorem b5_subps_pins :
  ([  -- 1 of hwprobe's 12 rows
    ...
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .sub false .x0 .x1, 3⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide
...
theorem b5_subpd_pins :
  ([  -- 1 of hwprobe's 10 rows
    ...
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .sub true .x0 .x1, 4⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide

theorem b5_divps_pins :
  ([  -- 9 of hwprobe's 14 rows
    (0x1f80, 0x7fc000007f7fffff400000003f800000, 0x007fffff3f0000003f80000040400000, 0x7fc000007f800000400000003eaaaaab, 0x28),  -- divps_mix/nearest · no vector reaches
    (0x1f80, 0x4000000040000000400000007f800123, 0x3f8000003f8000003f8000003f800000, 0x4000000040000000400000007fc00123, 0x01),  -- divps_loud@0 · no vector reaches
    (0x1f80, 0x40000000400000007f80012340000000, 0x3f8000003f8000003f8000003f800000, 0x40000000400000007fc0012340000000, 0x01),  -- divps_loud@1 · no vector reaches
    (0x1f80, 0x400000007f8001234000000040000000, 0x3f8000003f8000003f8000003f800000, 0x400000007fc001234000000040000000, 0x01),  -- divps_loud@2 · no vector reaches
    (0x1f80, 0x7f800123400000004000000040000000, 0x3f8000003f8000003f8000003f800000, 0x7fc00123400000004000000040000000, 0x01),  -- divps_loud@3 · no vector reaches
    (0x1f80, 0x4000000040000000007fffff7fc00000, 0x3f8000003f8000003f800000007fffff, 0x4000000040000000007fffff7fc00000, 0x02),  -- divps_de_split · no vector reaches
    (0x1f80, 0x4000000040000000007fffff007fffff, 0x3f8000003f8000003f80000000000000, 0x4000000040000000007fffff7f800000, 0x06),  -- divps_ze_de · no vector reaches
    (0x1f80, 0x400000004000000040000000007fffff, 0x3f8000003f8000003f80000000000000, 0x4000000040000000400000007f800000, 0x04),  -- divps_ze_only · no vector reaches
    (0x1f80, 0x00000000400000004000000040000000, 0x000000003f8000003f8000003f800000, 0xffc00000400000004000000040000000, 0x01)  -- divps_indef@3 · x86isa differs
  ] : List (BitVec 32 × BitVec 128 × BitVec 128 × BitVec 128 × BitVec 32)).all
    (fun (mx, a, b, r, fl) =>
      let t := step ⟨.vparith .div false .x0 .x1, 3⟩ (b5Pre mx a b)
      b5Agrees t mx fl (t.getXmm .x0) r) = true := by decide
